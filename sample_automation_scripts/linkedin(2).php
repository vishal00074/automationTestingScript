...
private function processFunCaptchaByClicking() {
    ...
    if ($this->exts->exists("iframe#captcha-internal")){
        ...
        if ($this->exts->exists('#CaptchaFrame')){
            ...
            if ($this->exts->exists("iframe[src*='client-api.arkoselabs']")){
            	$this->switchToFrame("iframe[src*='client-api.arkoselabs']");
            	sleep(1);
            }
            if ($this->exts->exists("iframe[src*='client-api.arkoselabs']")){
            	$this->switchToFrame("iframe[src*='client-api.arkoselabs']");
            ...
            if ($this->exts->exists("iframe[id='fc-iframe-wrap']")){
            	$this->switchToFrame("iframe[id='fc-iframe-wrap']");
            	sleep(1);
            }
            if ($this->exts->exists("iframe#CaptchaFrame")){
            	$this->switchToFrame("iframe#CaptchaFrame");
            	sleep(1);
            }

            

            $captcha_wraper_selector = 'iframe#CaptchaFrame';
            ...
            if($coordinates != ''){
                ...
                $this->exts->moveToElementAndClick($wraper, intval($coordinates[0]['x']), intval($coordinates[0]['y']));
                sleep(3);
            }
        }
        $this->exts->switchToDefault();
