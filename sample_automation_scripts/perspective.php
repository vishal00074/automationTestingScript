<?php // updated login code added clearcookies function and open login url again if user not logged in by cookies
// added code to accecpt cookies
...
class PortalScriptCDP
{
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            sleep(5);
            for ($i = 0; $i < 15 && $this->exts->getElement("//button[text()='See invoices' or contains(text(), 'Rechnungen ansehen')]") == null; $i++) {
                sleep(1);
            }
            $invoiceBtn = $this->exts->queryXpath("//button[text()='See invoices' or contains(text(), 'Rechnungen ansehen')]");
            if ($invoiceBtn) {
                $this->exts->click_element($invoiceBtn);
    ...
    {
        $this->waitForSelectors("div.z-modal div.border", 20, 2);
        if ($this->exts->exists('div.z-modal button.bump')) {
            $this->exts->click_element('div.z-modal button.bump');
            sleep(5);
        }
        $this->exts->capture("4-invoices-page");
        $invoices = [];

        $rows = $this->exts->querySelectorAll('div.z-modal div.border');
        foreach ($rows as $row) {
            if ($this->exts->querySelector('div:nth-child(2) > button:nth-child(2)', $row) != null) {
                $invoiceUrl = '';
                $text = $this->exts->extract('div:nth-child(1) >p', $row);
                preg_match('/^(.+) Â· (.+)$/', $text, $matches);
                $invoiceName = '';
                $invoiceDate = trim($matches[1]);
                $invoiceAmount = trim($matches[2]);;
                $downloadBtn = $this->exts->querySelector('div:nth-child(2) > button:nth-child(2)', $row);
