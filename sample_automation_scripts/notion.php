<?php // handle empty invoiceName and remove unncessary code
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 68981 - Last modified: 11.08.2025 23:47:00 UTC - User: 1
    ...
    public $check_login_success_selector = '.notion-sidebar .notion-sidebar-switcher, .notion-sidebar';
    ...
    private function initPortal($count)
    {
        ...
        for ($wait = 0; $wait < 15 && $this->exts->executeSafeScript("return !!document.querySelector('.notion-onboarding-workspace > [role=\"button\"]');") != 1; $wait++) {
        ...
        // i have hardcoded manual wait for element as it throws read timout error in test engine
        for ($wait = 0; $wait < 10
            && $this->exts->executeSafeScript("return !!document.querySelector('" . $this->check_login_success_selector . "');") != 1; $wait++) {
    ...
    private function googleFillTwoFactor($input_selector, $message_selector, $submit_selector, $submit_by_enter = false)
    {
        ...
        if (!empty($two_factor_code) && trim($two_factor_code) != '') {
            if ($this->exts->querySelector($input_selector) != null) {
                ...
                $this->exts->log('>>>>>>>>>>>>>>>>>>>>>> tabs');
                $this->exts->log(count($this->exts->get_all_tabs()));


                if (count($this->exts->get_all_tabs()) == 1) {
                    $this->exts->log('Switched to init');
                    $this->exts->switchToInitTab();
                }
    ...
    private function checkFillLogin()
    {
        ...
        $this->exts->capture(filename: "2-login-page");
        if ($this->exts->getElement($this->username_selector) != null) {
            sleep(3);
            $this->exts->log("Enter Username");
            ...
            try {
                ...
                $this->exts->waitTillPresent('div[role="alert"]', 30);
                if (strpos(strtolower($this->exts->extract('div[role="alert"]', null, 'innerText')), 'We could not reach the email address you provided') !== false) {
                    $this->exts->loginFailure(1);
                }
            } catch (Exception $e) {
                $this->exts->execute_javascript("arguments[0].click()", [$login_with_email_button]);
        ...
        $this->exts->waitTillPresent("//div[contains(text(), 'Iphone Auth')]");
        ...
        if ($this->exts->exists($this->onetime_code_selector)) {
        ...
        } else if ($this->exts->getElement($this->password_selector) != null) {
            $this->exts->log("Enter Password");
            ...
            $this->exts->waitTillPresent("//div[contains(text(), 'Iphone Auth')]", 40);
    ...
    private function checkFillLoginCode()
    {
        ...
        $two_factor_submit_selector = "//div[text()='Continue']";
    ...
    private function checkLoggedIn()
    {
        ...
        if ($this->exts->getElement($this->check_login_success_selector) != null) {
            ...
            if ($this->exts->exists('div[class="notion-dialog" ] div[role="button"]')) {
                $this->exts->click_element('div[class="notion-dialog" ] div[role="button"]');
            }
            ...
            if (strpos(strtolower($this->exts->extract($this->check_login_failed_selector, null, 'innerText')), 'Incorrect password') !== false) {
                $this->exts->loginFailure(1);
            } else if (strpos(strtolower($this->exts->extract('div[role="alert"]', null, 'innerText')), 'We could not reach the email address you provided. Please try again with a different email.') !== false) {
                $this->exts->loginFailure(1);
            } else if (!filter_var($this->username, FILTER_VALIDATE_EMAIL)) {
                $this->exts->loginFailure(1);
            } else if ($this->exts->exists('div[role="alert"]')) {
                $this->exts->loginFailure(1);
            } else {
    ...
    private function processMultipleWorkspaces()
    {
        ...
        if ($workspaces > 0) {
            for ($w = 1; $w <= $workspaces; $w++) {
                ...
                $this->exts->waitTillPresent('//div[text()="Settings" and @role="button"]', 25);
                sleep(10);
