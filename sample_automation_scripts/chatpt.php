<?php // replace exists and waitTillPresent
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 1808174 - Last modified: 29.04.2025 05:57:29 UTC - User: 1

    // Script here
    public $baseUrl = 'https://chat.openai.com/';
    public $username_selector = 'input#username, input#email-input, input[name="email"]';
    ...
    private function initPortal($count)
    {
        ...
        if (!$this->isLoggedin()) {
            ...
            $this->exts->click_element('button[data-testid="login-button"]');
            sleep(10);
            for ($i = 0; $i < 10 && $this->isExists('button[data-testid="login-button"]'); $i++) {
                $this->exts->click_by_xdotool('button[data-testid="login-button"]');
                sleep(5);
            }
            sleep(5);
            if ($this->exts->getElement('//button//*[contains(text(), "Log in")]', null, 'xpath') != null) {
            ...
            if ($this->login_with_google == '1') {
                if ($this->exts->urlContains('auth.openai.com/authorize') && !$this->isExists('//button[contains(text(), "Google")]')) {
                    $this->exts->refresh();
                    sleep(5);
                    $this->check_solve_cloudflare_page();
                }
                ...
                if ($this->exts->urlContains('auth.openai.com/authorize') && !$this->isExists('//button[contains(text(), "Apple")]')) {
                    $this->exts->refresh();
                    sleep(5);
                    $this->check_solve_cloudflare_page();
                }
                $this->exts->click_element('//button[contains(text(), "Apple")]');
                sleep(5);
                $this->loginAppleIfRequired();
                $this->check_solve_cloudflare_page();
            } else {
                for ($i = 0; $i < 4 && !$this->exts->oneExists([$this->username_selector, $this->password_selector]); $i++) {
                    $this->exts->refresh();
                    sleep(5);
                    $this->check_solve_cloudflare_page();
                    $this->exts->waitTillAnyPresent([$this->username_selector, $this->password_selector], 15);
                }
                $this->checkFillLogin();
                sleep(7);
                $this->check_solve_cloudflare_page();
            ...
            sleep(8);
            if ($this->exts->getElement('//button//*[contains(text(), "Personal account") or contains(text(), "nliches Konto")]') != null) {
                $this->exts->click_element('//button//*[contains(text(), "Personal account") or contains(text(), "nliches Konto")]');
                sleep(8);
            }
        }

        if ($this->isExists('button.bg-token-main-surface-primary') && $this->isExists('//h2//*[contains(text(), "Select a workspace")]')) {
            $this->exts->moveToElementAndClick('button.bg-token-main-surface-primary');
            sleep(8);
        }

        if ($this->isExists('nav[aria-label*="Chat"]') && !$this->isLoggedin()) {
            $this->exts->capture("seem-blank-page");
            $this->exts->refresh();
            sleep(8);
        }
        if ($this->isLoggedin()) {
            ...
            if ($this->isExists('[role="dialog"][data-state="open"] .text-token-text-tertiary button')) {
                $this->exts->click_element('[role="dialog"][data-state="open"] .text-token-text-tertiary button');
                sleep(1);
            }
            if ($this->isExists('[role="dialog"][data-state="open"]')) {
            ...
            $this->exts->moveToElementAndClick('button[data-testid="profile-button"]');
            sleep(1);
            $accountCount = count($this->exts->getElements('div[role="menuitem"] span:has(svg,img) + div'));
            if ($accountCount > 1) {
                for ($i = 0; $i < $accountCount; $i++) {
                    $this->exts->click_element($this->exts->getElements('div[role="menuitem"] span:has(svg,img) + div')[$i]);
                    sleep(3);
                    $this->waitFor('button[data-testid="profile-button"]');
                    $this->exts->openUrl('https://chatgpt.com/#settings/Subscription');
                    sleep(3);
                    $this->waitFor("(//button[contains(@class, 'btn-secondary')]//div[contains(text(), 'Manage')])[2]");
                    if ($this->isExists("(//button[contains(@class, 'btn-secondary')]//div[contains(text(), 'Manage')])[2]")) {
                        $this->exts->click_element("(//button[contains(@class, 'btn-secondary')]//div[contains(text(), 'Manage')])[2]");
                        $this->processInvoices();
                    }

                    //Invoice in admin page
                    $this->exts->openUrl('https://chatgpt.com/admin/billing');
                    $this->processAdminInvoices();
                    $this->exts->openUrl('https://chatgpt.com');
                    sleep(3);
                    $this->waitFor('button[data-testid="profile-button"]');
                    $this->exts->moveToElementAndClick('button[data-testid="profile-button"]');
                    sleep(1);
            ...
            if ($this->isExists('#error-element-password[data-error-code]')) {
                $this->exts->loginFailure(1);
            } elseif (stripos($this->exts->extract('div[class*="subtitle"]'), 'error=identity_provider_mismatch')) {
    ...
    private function checkFillLogin()
    {
        ...
        if ($this->exts->querySelector($this->username_selector) != null) {
            ...
            $this->exts->moveToElementAndClick('form._form-login-id button[type="submit"], button.continue-btn, input.continue-btn, button[type="submit"]');
            sleep(5);
            $this->check_solve_cloudflare_page();
        }
        ...
        if ($this->exts->querySelector($this->password_selector) != null) {
            ...
            $this->exts->click_element('//button[(text() = "Continue")]');
        } else {
    ...
    {
        $two_factor_selector = 'input#code, input[autocomplete="one-time-code"]';
        $two_factor_message_selector = 'header p, div[class*="_loginChallengePage"] p';
    ...
    {
        return $this->isExists('[data-testid="profile-button"] img, [data-testid="profile-button"] svg, button[data-testid="profile-button"]') != null &&
            $this->exts->getElementByText('[role="dialog"][data-state="open"] h2', ["session has expired"], null, false) == null;
    }
    ...
    private function loginGoogleIfRequired()
    {
        if ($this->exts->urlContains('google.')) {
            ...
            $this->checkGoogleTwoFactorMethod();
            sleep(10);
            if ($this->isExists("//button[.//span[contains(text(),'Continue')]]")) {
                $this->exts->click_element("//button[.//span[contains(text(),'Continue')]]");
                sleep(10);
    ...
    {
        sleep(3);
        $this->waitFor('a[href*="invoice.stripe.com"]');
        $restrictPages = isset($this->exts->config_array["restrictPages"]) ? (int)@$this->exts->config_array["restrictPages"] : 3;
        for ($i = 0; $i < 10 && $restrictPages == 0; $i++) {
            $this->exts->click_if_existed('//*[text()="View more" or text()="Mehr anzeigen"]');
            sleep(7);
            if (!$this->isExists('//*[text()="View more" or text()="Mehr anzeigen"]')) {
                break;
            }
        }
        ...
        $invoice_links = $this->exts->getElements('a[href*="invoice.stripe.com"]');
        ...
        foreach ($invoices as $invoice) {
            ...
            sleep(2);
            $invoice_name = '';
            if ($this->isExists('//*[text()="Invoice number" or text()="Rechnungsnummer"]/..//following-sibling::td[1]')) {
                $invoice_name = $this->exts->queryXpath('//*[text()="Invoice number" or text()="Rechnungsnummer"]/..//following-sibling::td[1]')->getAttribute('innerText');
                $invoice_name = 'Invoice-' . trim($invoice_name);
                if ($this->exts->invoice_exists($invoice_name) || $this->exts->document_exists($invoice_name . '.pdf')) {
                    $this->exts->log('Invoice Existed: ' . $invoice_name);
                    continue;
                }
            }

            $invoiceFilename = !empty($invoice_name) ? $invoice_name . '.pdf' : '';

            $downloaded_file = $this->exts->click_and_download('//button//*[contains(text(), "Download invoice") or contains(text(), "Rechnung herunterladen")]/../..', 'pdf', $invoiceFilename);
    ...
    private function processAdminInvoices($paging_count = 1)
    {
        sleep(3);
        $this->waitFor('a[href*="invoice.stripe.com"]');
        $this->exts->capture("4-invoices-page");
        ...
        if (
            $restrictPages == 0 &&
            $paging_count < 50 &&
            $this->exts->getElement('div.flex-grow + button') != null
        ) {
            $paging_count++;
            $this->exts->moveToElementAndClick('div.flex-grow + button');
            sleep(5);
            $this->processAdminInvoices($paging_count);
        }
    }
}
