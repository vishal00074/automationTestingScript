<?php // i have added condition to handle empty invoice name added restrict page condition in pagiantion code.
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 52929 - Last modified: 04.03.2025 14:42:08 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->getElement($this->check_login_success_selector) != null) {
            ...
            $new_download = false;
            $this->exts->waitTillPresent('a[href*="/bill"]');
            if ($this->exts->exists('a[href*="/bill"]')) {
                $this->exts->moveToElementAndClick('a[href*="/bill"]');
                sleep(2);
                if ($this->exts->exists('a[href*="account/payments"]')) {
                    $this->exts->moveToElementAndClick('a[href*="account/payments"]');
                    sleep(15);
                    if ($this->exts->exists('div[role="menu"] ul li a')) {
                        $len_monthlies = count($this->exts->getElements('div[role="menu"] ul li a'));
                        $this->exts->moveToElementAndClick('button.dropdown-button.btn');
                        sleep(5);
                        if ($restrictPages != 0) {
                            for ($i = 0; $i < $len_monthlies; $i++) {
                                $mon = $this->exts->getElements('div[role="menu"] ul li a')[$i];
                                $text = strtolower($mon->getAttribute('innerText'));
                                if (strpos($text, '6') !== false) {
                                    try {
                                        ...
                                        $this->exts->executeSafeScript("arguments[0].click()", [$mon]);
                                    }
                                    sleep(15);
                                    $this->processInvoices(1, $new_download);
                                }
                            }
                        } else {
                            for ($i = 0; $i < $len_monthlies; $i++) {
                                ...
                                $text = strtolower($mon->getAttribute('innerText'));
                                $this->exts->log($text);
                                if (strpos($text, 'this year') !== false) {
                                    try {
                                        $this->exts->log('Click download button');
                                        $mon->click();
                                    } catch (\Exception $exception) {
                                    ...
                                    $this->exts->moveToElementAndClick('button.dropdown-button.btn');
                                    sleep(5);
                                } else if (strpos($text, 'last year') !== false) {
                                    try {
                                        $this->exts->log('Click download button');
                                        $mon->click();
                                    } catch (\Exception $exception) {
                        ...
                        $new_download = true;
                        //click button "View All monthly statements"
                        $this->exts->moveToElementAndClick('a[data-test-id="all-monthly-statements"]');
                        sleep(10);

                        $statementUrl = $this->exts->getUrl();
                        $this->exts->log($statementUrl);
                        $this->processInvoices(1, $new_download);
                    }
                }
            }

            $purchase = isset($this->exts->config_array["purchase_receipt"]) ? (int)@$this->exts->config_array["purchase_receipt"] : 0;
            if ($purchase == 1) {
                // Open purchase url
                $this->exts->openUrl($this->purchasePageUrl);
                $this->processPurchaseInvoices();
            }
    ...
    private function checkFillRecaptcha()
    {
        ...
        $recaptcha_iframe_selector = '.g-recaptcha-etsy iframe[src*="/recaptcha/"]';
        $recaptcha_textarea_selector = 'textarea[name="g-recaptcha-response"]';

        $this->exts->waitTillPresent($recaptcha_iframe_selector, 10);
    ...
    private function processPurchaseInvoices($count = 1, $paging_count = 1)
    {
        sleep(15);
        //Close banner if any
        if ($this->exts->getElement('div.banner-inner button') != null) {
            $this->exts->moveToElementAndClick('div.banner-inner button');
            sleep(1);
        }
        ...
        $body_height = $this->exts->execute_javascript('return document.querySelector("body").scrollHeight;');
        while ($currentPageHeight < $body_height) {
            $currentPageHeight += 200;
            $this->exts->execute_javascript('window.scrollTo(0,' . $currentPageHeight . ');');
            sleep(1);
            $body_height = $this->exts->execute_javascript('return document.querySelector("body").scrollHeight;');
        }
        sleep(10);
        ...
        $rows = $this->exts->getElements('li.receipt-section');
        foreach ($rows as $row) {
            if ($this->exts->getElement('.action-buttons a[href*="purchases/"][data-event="view-receipt"]:not([href*="help"]', $row) != null) {
                $invoiceUrl = $this->exts->getElement('.action-buttons a[href*="purchases/"][data-event="view-receipt"]:not([href*="help"]', $row)->getAttribute("href");
                $invoiceName = explode(
                    '?',
                    array_pop(explode('purchases/', $invoiceUrl))
                )[0];
                $invoiceDate = array_pop(explode('on ', $this->exts->getElement('span.second-line', $row)->getAttribute('innerText')));
                $invoiceAmount = trim(preg_replace('/[^\d\.\,]/', '', $this->exts->getElement('span.currency-value', $row)->getAttribute('innerText'))) . ' USD';
        ...
        foreach ($invoices as $invoice) {
            ...
            if ($date_parsed = '') {
                $date_parsed = $this->exts->parse_date($invoice['invoiceDate'], 'M j, Y', 'Y-m-d');
            }
            ...
            if ($this->exts->exists('.order-details-body-summary .invoice a[href*="/invoice"]')) {
                $download_url = $this->exts->getElement('.order-details-body-summary .invoice a[href*="/invoice"]')->getAttribute('href');
                $this->exts->log('Download URL - ' . $download_url);
            ...
            } else if ($this->exts->exists('div.print-links')) {
                $this->exts->moveToElementAndClick('div.print-links');
                sleep(2);

                if ($this->exts->invoice_exists($invoice['invoiceName'])) {
                    $this->exts->log('Invoice existed ' . $invoiceFileName);
                } else {
                    $this->exts->moveToElementAndClick('a[data-event="receipt-print"]');
                    sleep(5);
                    $this->exts->wait_and_check_download('pdf');
                    $downloaded_file = $this->exts->find_saved_file('pdf', $invoiceFileName);

                    if (trim($downloaded_file) != '' && file_exists($downloaded_file)) {
                        $this->exts->new_invoice($invoiceName, $date_parsed, $invoiceAmount, $downloaded_file);
                    } else {
                        $this->exts->log(__FUNCTION__ . '::No download ' . $invoiceFileName);
                    }
                }
            }
            //COmmented this because user want Official Invoice not just print of the page
            /*else {
        ...
        if (
            $restrictPages == 0 &&
            $paging_count < 50 &&
            $this->exts->getElement('a#pager-next:not(.next-disabled)') != null
        ) {
            $paging_count++;
            $this->exts->moveToElementAndClick('a#pager-next:not(.next-disabled)');
            sleep(5);
            $this->processPurchaseInvoices(1, $paging_count);
        } else if ($restrictPages > 0) {
            if ($paging_count < $restrictPages && $this->exts->exists('a#pager-next:not(.next-disabled)')) {
                $this->exts->moveToElementAndClick('a#pager-next:not(.next-disabled)');
                sleep(7);
                $paging_count++;
                $this->processPurchaseInvoices(1, $paging_count);
    ...
    private function processInvoices($count = 1, $new_download = true)
    {
        sleep(5);
        if ($this->exts->getElement('table#totals-by-month tbody tr a[href*="/monthly-statement?"], [data-key="totals-by-month-row"] a[href*="/payments/monthly-statement?month"]') != null && !$new_download) {
            $this->exts->log('Invoices found');
            $this->exts->capture("4-page-opened");
            $invoices = [];

            if ($this->exts->exists('[data-key="totals-by-month-row"]')) {
                $rows = $this->exts->getElements('[data-key="totals-by-month-row"]');
                foreach ($rows as $row) {
                    $tags = $this->exts->getElements('[class*="wt-grid__item"]', $row);
                    // if(count($tags) < 3){
                    //     continue;
                    // }
                    $as = $this->exts->getElements('a[href*="/payments/monthly-statement?month"]', $row);
                    if (count($as) == 0) {
                        continue;
                    }
                    ...
                    $invoiceName = explode(
                        '&',
                        array_pop(explode('month=', $invoiceUrl))
                    )[0] . '-' . explode(
                        '&',
                        array_pop(explode('year=', $invoiceUrl))
                    )[0];
                ...
                $rows = $this->exts->getElements('table#totals-by-month tbody tr');
                foreach ($rows as $row) {
                    $tags = $this->exts->getElements('td', $row);
                    // if(count($tags) < 3){
                    //     continue;
                    // }
                    $as = $this->exts->getElements('a[href*="/monthly-statement?"]', $tags[0]);
                    if (count($as) == 0) {
                        continue;
                    }
            ...
            foreach ($invoices as $invoice) {
                ...
                $invoice['invoiceDate'] = $this->exts->parse_date($invoice['invoiceDate'], 'm-Y', 'Y-m-d');
                $this->exts->log('invoiceName: ' . $invoice['invoiceName']);
                $this->exts->log('invoiceDate: ' . $invoice['invoiceDate']);
                $this->exts->log('invoiceAmount: ' . $invoice['invoiceAmount']);
                $this->exts->log('invoiceUrl: ' . $invoice['invoiceUrl']);
                ...
                if ($this->exts->invoice_exists($invoice['invoiceName'])) {
                    ...
                    $this->exts->openUrl($invoice['invoiceUrl']);
                    sleep(7);
                    if ($this->exts->getElement('a[href*="vat_statement.php"]') != null) {
                        $this->exts->log('Dowloading invoice ' . $count . '/' . $totalFiles);

                        $this->exts->moveToElementAndClick('a[href*="vat_statement.php"]');
                        ...
                        $this->exts->capture('no-pdf' . $invoice['invoiceName']);
                    }
                }
            }
        } else {
            ...
            for ($j = 0; $j < $limit; $j++) {
                $month = date('n', strtotime('-' . $j . ' months'));
                $year = date('Y', strtotime('-' . $j . ' months'));

                $this->exts->log('year: ' . $year);
                $this->exts->log('month: ' . $month);

                $invoiceUrl = 'https://www.etsy.com/your/account/payments/monthly-statement?month=' . $month . '&year=' . $year;
                $invoiceName = $month . '_' . $year;
            ...
            foreach ($invoices as $invoice) {
                ...
                $invoice['invoiceDate'] = $this->exts->parse_date($invoice['invoiceDate'], 'n/Y', 'Y-m-d');
                $this->exts->log('invoiceName: ' . $invoice['invoiceName']);
                $this->exts->log('invoiceDate: ' . $invoice['invoiceDate']);
                $this->exts->log('invoiceAmount: ' . $invoice['invoiceAmount']);
                $this->exts->log('invoiceUrl: ' . $invoice['invoiceUrl']);
