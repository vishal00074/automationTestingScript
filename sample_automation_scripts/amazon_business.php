...
private function initPortal($count)
{
    ...
    if ($this->checkLogin()) {
        ...
        if ($this->isIncorrectCredential() || $this->exts->exists('div#auth-email-invalid-claim-alert') || $this->exts->exists('div[class*="invalid-code"]:not([class*="cvf-hidden"])')) {
...
function processYears($optionSelectors)
{
    ...
    foreach ($optionSelectors as $optionSelector) {
        ...
        for ($paging_count = 1; $paging_count < 100; $paging_count++) {
            if ($this->exts->querySelector(".order-card, .js-order-card") != null) {
...
public function download_pay_invoice($count = 0, $page = 1)
{
    $this->exts->update_process_lock();
