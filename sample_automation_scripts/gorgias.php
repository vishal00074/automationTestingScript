<?php // handle empty invoices name data mismatch
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 51056 - Last modified: 27.02.2025 12:59:48 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->querySelector($this->check_login_success_selector) != null) {
            ...
            $this->exts->openUrl($currentDomainUrl . '/app/settings/billing');
            $this->exts->waitTillPresent('a[href*="payment-history"]', 20);
    ...
    private function processInvoices()
    {
        ...
        foreach ($rows as $row) {
            ...
            if (count($tags) >= 5 && $this->exts->querySelector('a[href*="/invoice/"]', $tags[4]) != null) {
                // https://pay.stripe.com/invoice/acct_18qJaKI9qXomtXqS/invst_I0YvlQRdC84FeLh3PNoo1jEKMDOUiwU/pdf
                $invoiceUrl = $this->exts->querySelector('a[href*="/invoice/"]', $tags[4])->getAttribute("href");
                $invoiceName = explode(
                    '/',
        ...
        $date_from = $restrictPages == 0 ? strtotime('-2 years') : strtotime('-1 years');
