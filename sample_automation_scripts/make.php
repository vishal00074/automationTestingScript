...
// added check_solve_cloudflare_page to solve blocked page added  restrictPages condition to download inovices
// added code to trigger success
/**
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 1193635 - Last modified: 29.08.2025 03:43:21 UTC - User: 1

    // Start Script

    public $baseUrl = 'https://www.make.com/en/login';
    public $loginUrl = 'https://www.make.com/en/login';
    public $invoicePageUrl = '';
    public $username_selector = 'input[name="email"]';
    public $password_selector = 'input[name="password"]';
    public $remember_me_selector = '';
    public $submit_login_selector = 'button.btn[type="submit"]';
    public $check_login_failed_selector = 'dmo-el-static-message[title="Invalid credentials"]';
    public $check_login_success_selector = 'a[href*="/organization/"],a[href*="/logout"],.navbar-nav a[href*="/organization/"]';
    ...
    private function initPortal($count)
    {
        ...
        //Check if user opted for password save or not and if not then do the login otherwise not
        if (empty($this->exts->config_array["without_password"])) {
            // If user hase not logged in from cookie, clear cookie, open the login url and do login
            $this->check_solve_blocked_page();
            sleep(10);
            if ((int) @$this->login_with_google == 1) {
                $this->exts->moveToElementAndClick('a[href="/en/login/google?"]');
                sleep(10);
                ...
                $this->waitFor('input[name="tfaCode"]', 5);
                $this->waitFor('input[name="tfaCode"]', 5);
                if ($this->exts->exists('input[name="tfaCode"]')) {
                    $this->processTwoFactor(1);
                }
            }
        }
        ...
        if ($this->exts->exists($this->check_login_success_selector)) {
            ...
            if ($this->exts->exists('//header[text()="Invalid credentials"]') || $this->exts->exists($this->check_login_failed_selector)) {
    ...
    private function processTwoFactor($count = 1)
    {
        if ($this->exts->exists('input[name="tfaCode"]')) { // Authenticator app code
            $two_factor_selector = 'input[name="tfaCode"]';
            $two_factor_message_selector = '.content .mb-5:nth-child(2)';
            $this->exts->log("Two factor page found.");
            $this->exts->capture("2.1-two-factor-" . $count);

            if ($this->exts->getElement($two_factor_message_selector) != null) {
                $this->exts->two_factor_notif_msg_en = "";
                $messages = $this->exts->getElementsAttribute($two_factor_message_selector, 'innerText');
                $this->exts->two_factor_notif_msg_en = implode("\n", $messages);
            ...
            if (!empty($two_factor_code) && trim($two_factor_code) != '') {
                ...
                sleep(1);
                $this->exts->moveToElementAndClick('input[name*="wantsToRememberDevice2fa"]');
                sleep(1);
                $this->exts->moveToElementAndClick('button.btn[type="submit"]');
                sleep(12);
    ...
    private function processAfterLogin()
    {
        ...
        $multi_accounts = $this->exts->getElements("ul.select-base-items li a");

        $selected_accounts = []; // Initialize the array

        if (!empty($multi_accounts)) { // Check if $multi_accounts is not empty
            for ($j = 0; $j < count($multi_accounts); $j++) { // Fix loop condition
                $selected_accounts[] = $multi_accounts[$j]->getAttribute("href");

                sleep(10); // Retain delay if necessary
            }
            if (!empty($selected_accounts)) {
                print_r($selected_accounts); // Print the collected URLs

                foreach ($selected_accounts as $selected_account) {
                    $this->exts->openUrl($selected_account);
                    sleep(35); // Retain delay if necessary
                    if ($this->exts->exists('.nav.nav-tabs li a[href*="/organization/"][href*="/payments"]')) {
                        $this->exts->moveToElementAndClick('.nav.nav-tabs li a[href*="payments"],.nav.nav-tabs li a[href*="/organization/"][href*="/payments"]');
            ...
            if ($this->exts->exists('.nav.nav-tabs li a[href*="/organization/"][href*="/payments"]')) {
                $this->exts->moveToElementAndClick('.nav.nav-tabs li a[href*="/organization/"][href*="/payments"]');
                $this->processInvoices();
            }
        }
    ...
    {
        $this->waitFor('.table-content table tr', 5);
        $this->waitFor('.table-content table tr', 5);
        $this->waitFor('.table-content table tr', 5);
        ...
        $rows = $this->exts->getElements('.table-content table tr');
        foreach ($rows as $row) {
            ...
            if (count($tags) >= 8 && $this->exts->getElement('td:last-child button', $tags[7]) != null) {
                $invoiceBtn = $this->exts->getElement('td:last-child button', $tags[7]);
                $invoiceName = trim($this->exts->extract('td:nth-child(1)', $row));
                $this->exts->log('**** invoice name: ' . $invoiceName);
                $invoiceDate = trim($this->exts->extract('td:nth-child(2)', $row));
                ...
                try {
                    ...
                    $this->exts->log('ERROR in click_and_download. Could not locate element - ' . $exception);
                    $this->exts->log(__FUNCTION__ . '::No download ' . $invoiceFileName);
                }
            }
        }
...
$portal = new PortalScriptCDP("optimized-chrome-v2", 'Make', '2673551', 'aW5mb0BmcmVzZS1yZWNydWl0aW5nLmRl', 'RnJlc2VSZWNydWl0aW5nXzIwMjI=');
$portal->run();
