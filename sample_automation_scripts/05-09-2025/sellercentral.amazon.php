<?php //  Connection refused on 2fa time
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 916 - Last modified: 01.09.2025 13:49:06 UTC - User: 1
    ...
    private function downloadSellerFeeInvoice()
    {
        $this->exts->log(__FUNCTION__);
        sleep(3);
        $this->waitFor('table > tbody > tr button[data-invoice]');
    ...
    private function downloadAdvertiserInvoices($partnerId = '')
    {
        ...
        sleep(5);
        $this->waitFor('div[id="paid-invoices-table:table"] div.ag-center-cols-container div.ag-row button[data-takt-id*="SingleDownload"]');
            ...
            for ($i = 0; $i < $rows; $i++) {
                ...
                if ($download_button != null) {
                    ...
                    if ($this->exts->invoice_exists($invoiceName) || $this->exts->document_exists($invoiceFileName)) {
                        ...
                        sleep(1);
                        $this->waitFor('button[value*="INVOICE"]');
        ...
        for ($paging_count = 1; $paging_count < 50; $paging_count++) {
            ...
            foreach ($rows as $row) {
                ...
                if ($invoice_manage_button != null) {
                    ...
                    sleep(2);
                    $this->waitFor('.a-popover-modal[aria-hidden="false"] kat-table-body [role="row"]');
                    sleep(2);
