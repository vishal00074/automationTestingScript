...
// Server-Portal-ID: 6415 - Last modified: 07.02.2025 06:24:57 UTC - User: 1
        ...
        if ($this->exts->exists($this->username_selector) && $this->exts->exists($this->password_selector)) {
            ...
            if (!$this->exts->exists('#login-form div#password-entry:not(.hidden) input#password') && $this->exts->exists($this->submit_button_selector)) {
                ...
                if ($this->exts->exists($this->password_selector)) {
                    ...
                    sleep(5);
                    $this->exts->moveToElementAndClick($this->submit_button_selector);
                    $this->exts->waitTillPresent('div#login-error');
                ...
                $this->exts->log("Enter Password");
                sleep(5);
                $this->exts->moveToElementAndClick($this->password_selector, $this->password);
                sleep(5);
                $this->exts->moveToElementAndClick($this->submit_button_selector);
                sleep(15);
...
private function checkFillFormWithAlassian()
{
    if ($this->exts->querySelector('input#username') != null) {
        ...
        $this->exts->log("Enter Username");
        $this->exts->moveToElementAndClick('input#username', $this->username);
        sleep(2);
        ...
        $this->exts->log("Enter Password");
        $this->exts->moveToElementAndClick('input#password', $this->password);
        sleep(2);

        $this->exts->capture("2-login-page-filled");
        $this->exts->moveToElementAndClick('button#login-submit');
        $this->exts->waitTillPresent('div#login-error',10);
        if ($this->exts->exists('div#login-error')) {
            $this->exts->loginFailure(1);
        }
    } else {
...
private function checkFillFormWithOktaAlassian()
{
    if ($this->exts->querySelector('input#okta-signin-password') != null) {
        ...
        $this->exts->log("Enter Username");
        $this->exts->moveToElementAndClick('input#okta-signin-username', $this->username);
        sleep(2);

        $this->exts->log("Enter Password");
        $this->exts->moveToElementAndClick('input#okta-signin-password', $this->password);
        sleep(2);
...
{
    $two_factor_selector = 'input[data-testid="otp-input-index-0"]';
    $two_factor_message_selector = 'div[id="ProductHeadingSuffix"] h5';
...
function checkLogin()
{
    ...
    $isLoggedIn = false;
    try {
        if ($this->exts->exists('.content-all-boards')) {
            $this->exts->log(">>>>>>>>>>>>>>>Login successful!!!!");
            $isLoggedIn = true;
        }
    } catch (Exception $exception) {
...
function invoicePage()
{
    ...
    $this->exts->waitTillPresent('a[href*="/boards"]');
    $this->exts->moveToElementAndClick('a[href*="/boards"]');

    $this->exts->waitTillPresent('[data-testid="header-member-menu-button"]');
    $this->exts->moveToElementAndClick('[data-testid="header-member-menu-button"]');

    $this->exts->waitTillPresent('[data-testid="header-member-menu-popover"] a[href*="/billing"]');
    $this->exts->moveToElementAndClick('[data-testid="header-member-menu-popover"] a[href*="/billing"]');

    $this->exts->waitTillPresent('.tabbed-pane-main-col-wrapper a[href*="/billing"]');

    if (!$this->exts->exists('#billing-statement > div') && !$this->exts->exists('section table tbody tr[data-test-id="billing-history-statement"]') && $this->exts->exists('.tabbed-pane-main-col-wrapper a[href*="/billing"]')) {
        $this->exts->moveToElementAndClick('.tabbed-pane-main-col-wrapper a[href*="/billing"]');
        sleep(15);
    }
    $this->downloadInvoice();
...
function downloadInvoice($count = 1, $pageCount = 1)
{
    ...
    try {
        $this->exts->waitTillPresent('#billing-statement > div', 20);
