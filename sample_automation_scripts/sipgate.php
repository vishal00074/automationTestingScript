				...
				if(count($columns) > 0) {
					try {
						...
						if(trim($invoice_url) != "") {
							...
							if(stripos($invoice_amount, "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬") !== false && stripos($invoice_amount, "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬") <= 1) {
								$invoice_amount = preg_replace('/[^\d\.,-]/', '', $invoice_amount);
								$invoice_amount = trim($invoice_amount).' EUR';
							} else if(stripos($invoice_amount, "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬") !== false && stripos($invoice_amount, "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬") > 1) {
								$invoice_amount = preg_replace('/[^\d\.,-]/', '', $invoice_amount);
								$invoice_amount = trim($invoice_amount).' EUR';
								...
								$invoice_amount = preg_replace('/[^\d\.,-]/', '', $invoice_amount);
								$invoice_amount = trim($invoice_amount).' USD';
							} else if(stripos($invoice_amount, "ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£") !== false && stripos($invoice_amount, "ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£") <= 1) {
								$invoice_amount = preg_replace('/[^\d\.,-]/', '', $invoice_amount);
			...
			if(!empty($invoiceDataArr) && count($invoiceDataArr) > 0) {
				foreach($invoiceDataArr as $invoiceData) {
					...
					if(trim($invoiceData['invoice_name']) != "") {
						$filename = $invoiceData['invoice_name'].".pdf";
					}
