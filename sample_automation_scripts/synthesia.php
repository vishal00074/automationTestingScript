...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 1781486 - Last modified: 12.07.2025 17:06:52 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            sleep(10);
            $this->exts->click_element("//button[.//text()[contains(., 'Close')]]");
            sleep(10);
            $this->exts->click_element('button[aria-label="User profile"]');
            sleep(10);
            $this->exts->click_element('li#accountGeneral');
            sleep(10);
            $this->exts->click_element("//div[@role='button' and descendant::text()[normalize-space()='Billing' or normalize-space()='Rechnung']]");
            sleep(10);
            if ($this->exts->exists('//button[text()="Manage billing details"]')) {
                $this->exts->click_element('//button[text()="Manage billing details"]');
            } else {
                sleep(10);
                $this->exts->click_element("//span[contains(text(), 'Monthly') or contains(text(), 'Yearly') or contains(text(), 'Quarterly')]/following-sibling::button");
                sleep(2);
                $this->exts->click_element("//li[.//span[text()='Manage billing details']]");
            }
    ...
    private function processInvoices($paging_count = 1)
    {
        ...
        for ($paging_count = 0; ($paging_count < $maxAttempts && $this->exts->exists('button[data-testid="view-more-button"]')); $paging_count++) {
