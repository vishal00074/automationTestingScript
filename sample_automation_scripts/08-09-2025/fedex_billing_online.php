<?php // replace exists with custom js isExists 
// script execute successfully 
//on test engine but  on watch logs due to long script execetion getting Connection refused message after success
/**
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 137156 - Last modified: 03.09.2025 14:07:20 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if (!$this->checkLogin()) {
            ...
            for ($i = 0; $i <= 5; $i++) {
                ...
                if ($this->isExists('.js-modal-close')) {
                    $this->exts->moveToElementAndClick('.js-modal-close');
                    sleep(5);
                }

                if ($this->isExists('button.fxg-gdpr__accept-all-btn')) {
            ...
            ');
                ...
                if ($this->isExists('button#cancelBtn')) {
                    $this->exts->moveToElementAndClick('button#cancelBtn');
                    sleep(10);
                }
                if ($this->isExists('button#retry-btn')) {
                    $this->exts->moveToElementAndClick('button#retry-btn');
                    sleep(10);
                }
                if ((!$this->isExists($this->password_selector) && $this->isExists($this->check_login_success_selector)) || $this->isExists($this->check_login_failed_selector)) {
                    break;
                }
            }
        }
        ...
        if ($this->checkLogin()) {
            ...
            if ($this->isExists('button[aria-label*="close"]')) {
...
');
    ...
    private function invoicePage()
    {
        ...
        if ($this->isExists('td[data-label="invoiceNumber"]')) {
    ...
    private function processInvoices($pageCount = 1)
    {
        ...
        if ($this->isExists('div[class="fdx-c-dialog__main"]') && $this->exts->getElement('//h6[normalize-space(text()) = "There was an error processing your request, Please try again later."]') != null) {
        ...
        for ($i = 0; $i < $invoicesCount && $this->isExists('div.navigation > svg:nth-child(3):not(.cursor-not-allowed)'); $i++) {
