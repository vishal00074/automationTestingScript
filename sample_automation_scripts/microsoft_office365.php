<?php // replace exists to isExists function  added clearChrome function updated selector a[id="signInAnotherWay"] added account not ready in case scure screen appear
...
class PortalScriptCDP
{
	...
	// Server-Portal-ID: 391 - Last modified: 17.07.2025 15:41:58 UTC - User: 1
	...
	private function doAfterLogin()
	{
		...
		if ($this->isLoggedIn()) {
			...
			if ($this->isExists('#O365_NavHeader button#O365_MainLink_NavMenu')) {
			...
			if ($this->isExists('a#ShellAdmin_link[href*="admin.microsoft.com"], #appsModule a[href*="admin.microsoft.com"], a[href*="admin.microsoft.com"]')) {
				$this->exts->log('This user is office 365 user');
				$admin_url = $this->exts->extract('a#ShellAdmin_link[href*="admin.microsoft.com"], #appsModule a[href*="admin.microsoft.com"], a[href*="admin.microsoft.com"]', null, 'href');
				...
				$this->checkConfirmButton();
				if (!$this->isExists('ul[role="menubar"] li button[data-value="billing"], ul[role="menubar"] li button[data-value="Billing"]')) {
				...
				if (!$this->isExists('iframe#SFBIFrame, iframe#ContainerFrame, [data-automation-id="ListInvoiceList"]')) {
					sleep(30);
				}
				...
				if ($this->isExists('iframe#SFBIFrame')) {
					$this->downloadInvoices();
				} else if ($this->isExists('#adminAppRoot:not([class*="hide"]) iframe#ContainerFrame')) {
			...
			} else if ($this->isExists('a[href*="account.microsoft.com"]')) {
			...
			for ($wait = 0; $wait < 15 && $this->exts->executeSafeScript("return !!document.querySelector('button [data-automation-id=\'LeftNavBillingNavToggler\']');") != 1; $wait++) {
				$this->exts->log('Waiting for login.....');
				sleep(10);
			}

			if ($this->isExists('button[data-automation-id="LeftNavBillingNavToggler"]')) {
				$this->exts->moveToElementAndClick('a[href="#/billoverview"]');
				$this->exts->waitTillPresent('div[class*="entityInnerHost"]');
				if ($this->isExists('div[class*="entityInnerHost"]')) {
	...
	public function invoiceAndPayment()
	{
		...
		foreach ($rows as $row) {
			if ($this->exts->getElement('button[type="button"]', $row) != null) {
				$invoiceName = $this->exts->extract('div[data-automation-key="id"]', $row, 'innerText');
	...
	{
		$this->exts->log(__FUNCTION__);
		$this->exts->waitTillPresent('iframe#SFBIFrame', 30);
		$this->switchToFrame('iframe#SFBIFrame');
		sleep(5);
		if ($this->isExists('app-invoice-list fab-dropdown .ms-Dropdown')) {
	...
	private function downloadInvoicesClassic()
	{
		...
		if ($this->isExists('iframe#ContainerFrame')) {
	...
	private function downloadAccountOrders()
	{
		...
		foreach ($order_urls as $key => $order_url) {
			...
			if ($this->isExists('a#order-details-print')) {
				$invoiceName = explode(
					'&',
				...
				if ($this->isExists('a#orders-tax-invoice')) {
