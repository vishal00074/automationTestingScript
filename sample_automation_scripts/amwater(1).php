<?php // change wait till visible function to  present
...
private function initPortal($count) {
	...
	if(!$this->exts->exists($this->check_login_success_selector)) {
		...
		sleep(5);
		$this->exts->moveToElementAndClick('#login-form');
	...
	if($this->exts->exists($this->check_login_success_selector)) {
		...
		if ($this->exts->exists('div.premise-table div.premise-row button')){
			...
			for ($paging_count=0; $paging_count < 10; $paging_count++) {
				$accounts_count = count($this->exts->getElementsByCssSelector('div.premise-table div.premise-row'));
				for ($account_idx=0; $account_idx < $accounts_count; $account_idx++) { 
					$account_row = $this->exts->getElementsByCssSelector('div.premise-table div.premise-row')[$account_idx];
					if ($account_row == null) continue;
					$account_detail_button = $this->getElementByText('button', ['ACCOUNT DETAILS'], $account_row, false);
					if ($account_detail_button == null) continue;
					$this->exts->log(trim(strtolower($account_detail_button->getAttribute('innerText'))));
					$this->click_element($account_detail_button);
					sleep(5);
			...
			$paging_count = 0;
			for ($paging_count=0; $paging_count < 10; $paging_count++) {
				$accounts_count = count($this->exts->getElementsByCssSelector('div.myAccounts_IndividualCardId .webix_dataview_item button.accountDetailsBtn'));
				for ($account_idx=0; $account_idx < $accounts_count; $account_idx++) { 
...
private function processInvoices($paging_count=1) {
	sleep(10);
	// for ($wait_count = 1; $wait_count <= 10 && $this->exts->getElementByCssSelector('table > tbody > tr a[href*="/downloadinvoice') == null; $wait_count++) {
	...
	$rows = $this->exts->getElementsByCssSelector('table > tbody > tr');
	foreach ($rows as $index=>$row) {
		$tags = $this->exts->getElementsByCssSelector('td', $row);
		if(count($tags) >= 5 && $this->exts->getElementByCssSelector('label.view-bill-cursor', $tags[1]) != null) {
			$invoiceSelector = $this->exts->getElementByCssSelector('label.view-bill-cursor', $tags[1]);
			...
			if ($this->exts->document_exists($invoiceFileName)) {
				$this->exts->webdriver->executeScript("arguments[0].removeAttribute('id');", [$invoiceSelector]);
				continue;
			}
...
private function processInvoicesV2($paging_count=1) {
	...
	// $rows = $this->exts->getElementsByCssSelector('div[view_id*="billingAndPaymentsHistoryTable"] .webix_ss_body [column="1"]');
	$rows = count($this->exts->getElementsByCssSelector('div[view_id*="billingAndPaymentsHistoryTable"] .webix_ss_body [column="1"] [role="gridcell"]'));
	for ($i=0; $i < $rows; $i++) {
		$row = $this->exts->getElementsByCssSelector('div[view_id*="billingAndPaymentsHistoryTable"] .webix_ss_body [column="1"] [role="gridcell"]')[$i];
		$download_button = $this->exts->getElementByCssSelector('[onclick*="pdfView"]', $row);
