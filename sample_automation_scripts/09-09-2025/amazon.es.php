<?php // updated submit button selector in 2fa code 
// replace exists with custom js function isExists getting client read timout error on exists
/**
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 10251 - Last modified: 02.09.2025 13:14:46 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if (!$isCookieLoginSuccess) {
            ...
            if ($this->isExists('[action="/errors/validateCaptcha"] img')) {
                $this->fillForm(0);
                sleep(20);
            }

            if ($this->isExists('#auth-captcha-image-container img, [action="/errors/validateCaptcha"] img')) {
                $this->fillForm(0);
                sleep(20);
            }
        }
    ...
    private function fillForm($count)
    {
        ...
        try {
            ...
            if ($this->login_tryout == 0) {
                if ($this->exts->querySelector($this->password_selector) != null || $this->exts->querySelector($this->username_selector) != null) {
                    ...
                    if ($formType == null) {
                        ...
                        if ($this->isExists('#ap_captcha_guess, #auth-captcha-guess')) {
                            $this->exts->processCaptcha('#auth-captcha-image-container img', '#ap_captcha_guess, #auth-captcha-guess');
                            sleep(15);
                        } else if ($this->isExists('[action="/errors/validateCaptcha"] img')) {
                            ...
                            sleep(15);
                        } else if ($this->isExists('#auth-captcha-image-container img, [action="/errors/validateCaptcha"] img')) {
                            ...
                            $this->exts->click_by_xdotool('[action="/errors/validateCaptcha"] [type=submit]');
                            sleep(15);
                        } else if ($this->isExists('div#auth-error-message-box')) {
                            $this->exts->loginFailure(1);
                        }
                ...
                if ($this->isExists('form[action="verify"] input#continue')) {
                    $this->exts->click_by_xdotool('form[action="verify"] input#continue');
                    sleep(15);

                    if ($this->isExists('input[name="code"]')) {
                    ...
                    } else if ($this->isExists('#auth-mfa-otpcode')) {
            ...
            if ($this->isExists('div#auth-error-message-box div.a-alert-content')) {
                $this->exts->loginFailure(1);
            }

            $this->exts->capture('after-click-login');

            if ($this->isExists('input[name="verifyToken"]')) {
                ...
                if ($this->isExists('input[name="code"]')) {
                    ...
                    $this->checkFillTwoFactor('#auth-mfa-otpcode', '#auth-signin-button', 'div.a-row.a-spacing-none');
                } else if ($this->isExists('div#tiv-message + form[action="verify"]')) {
                    $this->checkFillTwoFactorWithPushNotify('div#tiv-message');
                } else if ($this->isExists('input[name="transactionApprovalStatus"]')) {
                    $this->checkFillTwoFactorWithPushNotify('div.a-section.a-spacing-large span.a-text-bold, div#channelDetails');
                }
            } else if ($this->isExists('[action="verify"]') && $this->isExists('[name*="dcq_question_date_picker"]')) {
                ...
                $this->checkFillTwoFactorWithPushNotify('div#tiv-message');
            } else if ($this->isExists('input[name="otpCode"]')) {
                $this->checkFillTwoFactor('input[name="otpCode"]', 'span#cvf-submit-otp-button input.a-button-input, input[name="mfaSubmit"]', 'form#auth-mfa-form div.a-box-inner > h1, form#auth-mfa-form div.a-box-inner > p');
            } else if ($this->isExists('div[data-a-input-name="otpDeviceContext"] input[value*="OTP"]')) {
            ...
            } else if ($this->isExists('div[data-a-input-name="otpDeviceContext"] input[value*="SMS"]')) {
                ...
                $this->checkFillTwoFactor('#auth-mfa-otpcode', '#auth-signin-button', 'form#auth-mfa-form div.a-box-inner > h1 ~ p');
            } else if ($this->isExists('div[data-a-input-name="otpDeviceContext"] input[value*="VOICE"]')) {
    ...
    private function checkFillTwoFactorWithPushNotify($two_factor_message_selector)
    {
        if ($this->exts->two_factor_attempts < 3) {
            ...
            if (!empty($two_factor_code) && trim($two_factor_code) != '' && strtolower($two_factor_code) == 'ok') {
                ...
                if ($this->exts->querySelector($two_factor_message_selector) == null && !$this->isExists('input[name="transactionApprovalStatus"]')) {
                    $this->exts->log("Two factor solved");
                } else if ($this->exts->two_factor_attempts < 3) {
                    $this->exts->two_factor_attempts++;
                    $this->exts->notification_uid = '';
    ...
    public function processAfterLogin($count)
    {
        ...
        if (stripos($this->exts->getUrl(), "amazon.es/ap/signin") === false) {
            ...
            if ($this->procurment_report == 1) {
                ...
                sleep(15);
                if ($this->isExists('#date_range_selector__range')) {
                    ...
                    if (!$this->isExists('.report-table .column:nth-child(3) [class*="cell-row-"]')) {
                        sleep(25);
                    }
    ...
    private function processYears($optionSelectors)
    {
        ...
        foreach ($optionSelectors as $optionSelector) {
            ...
            // if ($this->isExists("select[name=\"orderFilter\"]")) {
            ...
            $this->exts->log("processing year element  " . $optionSelector);
            if ($this->isExists('span[data-a-class="order-filter-dropdown"] span[role="button"], div#orderTimeFilter span.a-button-dropdown, form.js-time-filter-form span.a-button-dropdown')) {
            ...
            if ($this->exts->querySelector($this->password_selector) != null || stripos($this->exts->getUrl(), "/ap/signin?") !== FALSE) {
                if ($this->login_tryout == 0) {
                    $this->fillForm(0);
                    if ($this->isExists('a#ap-account-fixup-phone-skip-link')) {
            ...
            if (!$this->isExists("div.a-box-group.a-spacing-base.order, .order-card, #orderCard")) {
                sleep(15);
            }
            ...
            for ($paging_count = 1; $paging_count < 100; $paging_count++) {
                if ($this->exts->querySelector(".order-card, .js-order-card, div#yourOrderInfoSection #orderCard") != null) {
                                    ...
                                    if (count($columns) > 0) {
                                        ...
                                        if (trim($detailPageUrl) != "" && $this->dateLimitReached == 0) {
                                            ...
                                            if ($last_date_timestamp > strtotime($invoice_date) && $this->last_invoice_date != "" && !empty($this->last_invoice_date) && trim($parsed_date) != "") {
                                                ...
                                                if ($this->isExists('[data-action="a-popover-close"]')) {
                ...
                } else if ($this->isExists('.order-card a[href*="/ajax/invoice/"]')) {
                    ...
                    for ($i = 0; $i < $count_order_card; $i++) {
                        ...
                        sleep(5);
                        if ($this->isExists('.a-popover[style*="visibility: visible"]:not([aria-hidden="true"]) a[href*="/invoice.pdf"]')) {
                ...
                if ($this->isExists('.a-pagination li.a-selected + li:not(.a-disabled) a')) {
    ...
    public function processMSInvoice($currentMessagePage)
    {
        ...
        if ($this->exts->querySelector("#inbox_button_next_page") != null && (int)@$this->msgTimeLimitReached == 0) {
            ...
            if (stripos($nextBtnClass, "disabled") === FALSE) {
                ...
                if (!$this->isExists('span#inbox_button_next_page.a-button-disabled')) {
    ...
    public function download_procurment_document($pageNum = 1)
    {
        ...
        do {
            if ($pageNum > 1) {
                if ((int)$this->restrictPages == 0) {
                    if ($this->isExists('.report-table-footer button[data-testid="next-button"]') && !$this->isExists('.report-table-footer button[data-testid="next-button"][disabled]')) {
                    ...
                    if ($this->isExists('.report-table-footer button[data-testid="next-button"]') && !$this->isExists('.report-table-footer button[data-testid="next-button"][disabled]') && $pageNum < 50) {
                        $this->exts->click_by_xdotool('.report-table-footer button[data-testid="next-button"]');
                        sleep(15);
                        $pageNum++;
                    } else {
            ...
            foreach ($rows as $key => $row) {
                ...
                if ($linkBtn != null) {
                    ...
                    if ($this->isExists('.a-popover .a-popover-content a')) {
                        ...
                        foreach ($downloadLinks as $downloadLink) {
                            if (stripos($downloadLink, '/b2b/aba/order-summary/') !== false) {
                                if (trim($orderNum) !== '' && !$this->exts->invoice_exists($orderNum)) {
                                    ...
                                    $this->exts->openUrl($this->baseUrl);
                                    sleep(2);
                                    if ($this->isExists('a[href="/gp/css/homepage.html/ref=nav_bb_ya"]')) {
        ...
        } while ($this->isExists('.report-table-footer button[data-testid="next-button"]') && !$this->isExists('.report-table-footer button[data-testid="next-button"][disabled]') && $pageNum > 1);
    }
}
