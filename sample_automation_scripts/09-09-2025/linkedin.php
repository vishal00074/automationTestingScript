<?php // added configuration for security_phone_number  and recovery_email in google login code 
// script was failed due to Browser creation failure
// uncommented clearCookies function added notification_uid in checkTwoFactorAuth and updated two_factor_msg selector
// added pagiantion code in processInvoicestransactions function
/**
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 331 - Last modified: 26.08.2025 13:17:57 UTC - User: 1
    ...
    private function checkTwoFactorAuth($count = 1)
    {
        ...
        if ($is_recaptcha != null && stripos($this->exts->getUrl(), '/uas/consumer-captcha-v2?challengeId') !== false) {
            ...
            $ele = $this->exts->getElement('main.app__content h1.content__header');
            if ($ele != null) {
                $two_factor_msg = $ele->getText();
            }
            $this->handleTwoFactorCode($two_factor_input_selector, $two_factor_msg, $submit_button_selector);
...
";
    ...
    private function processInvoices($paging_count = 1)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (count($tags) >= 4 && $this->exts->getElement('button.view-receipt[data-order-id]', $row) != null) {
                $invoiceName = $this->exts->getElement('button.view-receipt[data-order-id]', $row)->getHtmlAttribute('data-order-id');
    ...
    public function downloadInvoices()
    {
        ...
        $this->exts->log('Download Invoices');
        ...
        foreach ($this->invoices as $invoice) {
            ...
            if ($this->company_detail != '') {
                if ($this->isExists('.company-billing-info-updater button.add-billing-details-btn:not(.hidden)')) {
                    ...
                    $this->exts->type_text_by_xdotool($this->company_detail);
                    sleep(1);
                    $this->exts->moveToElementAndClick('.company-billing-info-updater button.company-button[type="submit"]');
                } else if ($this->isExists('.company-billing-info-updater a.edit:not(.hidden)')) {
    ...
    private function processInvoicestransactions($count = 0)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (count($tags) >= 7 && $this->exts->getElement('button[id*="menu-trigger"]', $tags[6]) != null) {
                ...
                sleep(2);
                if ($this->isExists('a[download*="LNKD_INVOICE"]')) {
        ...
        $pagiantionSelector = 'button[class*="next-btn"]:not(:disabled)';
        if ($this->restrictPages == 0) {
            if ($count < 50 && $this->exts->querySelector($pagiantionSelector) != null) {
                ...
                sleep(7);
                $count++;
                $this->processInvoicestransactions($count);
            }
        } else {
