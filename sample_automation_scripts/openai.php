...
public $username_selector = 'input#email-input, input#username';
...
{
    $this->exts->log('open login page');

    $this->exts->openUrl($this->loginUrl);
         
    $this->exts->waitTillPresent('div[class="docs-scroll-container"] button');

    for ($i=0; $i < 4 && !$this->exts->exists('div[class="docs-scroll-container"] button'); $i++) { 

        $this->exts->log('Finding Login page: '. $i);

        $this->exts->refresh();
        $this->exts->waitTillPresent('div[class="docs-scroll-container"] button', 10);
    }

    $this->exts->click_by_xdotool('div[class="docs-scroll-container"] button');
}
private function checkFillLogin() {
    ...
    if($this->exts->querySelector($this->username_selector) != null) {
        ...
        $this->exts->click_by_xdotool('input[type="submit"], form._form-login-id button[type="submit"], button.continue-btn');
        sleep(5);
        $this->check_solve_cloudflare_page();
    }
    ...
    if($this->exts->querySelector($this->password_selector) != null) {
        ...
        $this->exts->click_by_xdotool('button._button-login-password');
        sleep(5);
    } else {
...
private function checkFillTwoFactor() {
    ...
    $two_factor_submit_selector = '[class*="loginChallengePage"] form button[type="submit"], button[type="submit"][data-action-button-primary="true"]';
...
private function isLoggedin() {
    $this->exts->openUrl($this->baseUrl);
    // return $this->exts->exists('a[href="/settings"], #root > div:first-child button[aria-haspopup="menu"]') != null && 
    //     $this->exts->getElementByText('[role="dialog"][data-state="open"] h2', ["session has expired"], null, false) == null;
    $this->exts->waitTillPresent('a[href="/settings/profile"]', 10);
    $isLoggedin = false;
