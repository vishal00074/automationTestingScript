<?php // update waittillpresent to waitFor and increase sleep time before loginurl opening and after login  add condition for dialog box if exists and then moveToElementAndClick 
...
class PortalScriptCDP
{
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            if ($this->exts->exists('[role="dialog"] button[data-automation-class*="x-btn"]')) {
                $this->exts->moveToElementAndClick('[role="dialog"] button[data-automation-class*="x-btn"]');
            }
            sleep(10);
            $this->exts->openUrl($this->invoicePageUrl);
            $this->waitFor('form[action*="create_customer_portal_session"] > button', 15);
            if ($this->exts->exists('form[action*="create_customer_portal_session"] > button')) {
                $this->exts->click_by_xdotool('form[action*="create_customer_portal_session"] > button');
            }
    ...
    private function processInvoices($paging_count = 1)
    {
        ...
        while ($attempt < $maxAttempts && $this->exts->exists('button[data-testid="view-more-button"]')) {
            $this->exts->click_by_xdotool('button[data-testid="view-more-button"]');
            $attempt++;
            sleep(5);
        }
        $invoices = [];
