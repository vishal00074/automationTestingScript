<?php // handle client read timeout on exists in download_procurment_document
/**
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 394 - Last modified: 04.08.2025 08:05:16 UTC - User: 9

    /*Define constants used in script*/
    // Server-Portal-ID: 394 - Last modified: 04.08.2025 08:05:16 UTC - User: 9

    /*Define constants used in script*/
    public $baseUrl = "https://www.amazon.de";
    public $orderPageUrl = "https://www.amazon.de/gp/css/order-history/ref=nav_youraccount_orders";
    public $messagePageUrl = "https://www.amazon.de/gp/message?ref_=ya_d_l_msg_center#!/inbox";
    public $alt_messagePageUrl = "https://www.amazon.de/gp/message?ie=UTF8&ref_=ya_mc_bsm&#!/inbox";
    public $businessPrimeUrl = "https://www.amazon.de/businessprime";
    ...
    public $only_business_invoice = 0;
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->docker_restart_counter == 0) {
            ...
            $this->only_business_invoice = isset($this->exts->config_array["only_business_invoice"]) ? (int) $this->exts->config_array["only_business_invoice"] : 0;
            ...
            if (!empty($this->invalid_filename_keywords)) {
                ...
                foreach ($this->invalid_filename_keywords as $s) {
                    if ($this->invalid_filename_pattern != '')
                        $this->invalid_filename_pattern .= '|';
        ...
        if ($this->checkLogin()) {
            ...
            $this->exts->openUrl('https://www.amazon.de/gp/message');
            sleep(5);
            $this->check_solve_captcha_page();
        ...
        if (!$this->checkLogin()) {
            ...
            sleep(5);
            if ($this->exts->oneExists([$this->password_selector, 'input#auth-captcha-guess']) && !$this->isIncorrectCredential()) {
                $this->fillForm(0);
                sleep(5);
                $this->checkFillTwoFactor();
            }
            ...
            // Choose Business Account by default if user has multi accounts atb(acccount type business)
            $this->exts->waitTillPresent('div[action*="switch_atb"] > a');
            if ($this->exts->exists('div[action*="switch_atb"] > a')) {
                $this->exts->click_element('div[action*="switch_atb"] > a');
                sleep(10);
            }
        }
        ...
        if ($this->checkLogin()) {
            ...
            if ($this->isIncorrectCredential() || $this->exts->exists('div#auth-email-invalid-claim-alert') || $this->exts->exists('div[class*="invalid-code"]:not([class*="cvf-hidden"])') || stripos(strtolower($this->exts->extract('div#approvalPollAlerts div[role="alert"]')), 'dein Code ist abgelaufen') !== false) {
                $this->exts->loginFailure(1);
            } elseif (stripos(strtolower($this->exts->extract('div[role="alert"]')), 'verification of your billing information required') !== false || stripos(strtolower($this->exts->extract('div[role="alert"]')), 'verifizierung deiner rechnungsdaten erforderlich') !== false) {
    ...
    private function fillForm($count)
    {
        ...
        $account_switcher_elements = $this->exts->querySelectorAll("div.cvf-account-switcher-profile-details-after-account-removed, a[data-name='switch_account_request']");
        ...
        if ($this->exts->urlContains('google.com/')) {
        ...
        } else if ($this->exts->exists('input#okta-signin-username') || $this->exts->exists('input#okta-signin-password')) {
            $this->checkFillExternalLogin();
            ...
            if ($this->exts->querySelector($this->password_selector) != null || $this->exts->querySelector($this->username_selector) != null) {
                ...
                if ($formType == null) {
                    ...
                    if ($this->exts->exists('div[id="invalid-email-alert"]:not([class*="hidden"])')) {
                    ...
                    if ($this->exts->urlContains('google.com/')) {
                        ...
                        if ($this->exts->exists($this->password_selector)) {
                            ...
                            $this->exts->type_key_by_xdotool('Return');
                            $this->exts->waitTillPresent('div#auth-error-message-box');
                            if ($this->isIncorrectCredential()) {
                                $this->exts->log("Wrong credential !!!!");
                                $this->exts->loginFailure(1);
                            }
                            $this->exts->capture("1-after-submit");
                    ...
                    $this->exts->capture("2-after-submit");
                    if ($this->exts->exists($this->submit_button_selector)) {
                        if ($this->exts->exists($this->password_selector)) {
                            $this->exts->log("Enter Password Again");
                            $this->exts->click_by_xdotool($this->password_selector);
    ...
    private function isIncorrectCredential()
    {
        $incorrect_credential_keys = [
            ...
            'La tua password non',
            'a no es correcta',
            'Dein Einmalpasswort (OTP) ist abgelaufen',
            'passwor'
        ];
    ...
    private function checkLogin()
    {
        ...
        try {
            if ($this->exts->querySelector($this->logout_link) != null) {
                ...
                } else if ($this->exts->querySelector("a#nav-item-signout-sa, a[data-name='sign_out_request']") != null) {
                    $isLoggedIn = true;
    ...
    private function checkFillMicrosoftLogin()
    {
        ...
        if ($this->exts->exists('div#idDiv_RemoteNGC_PollingDescription')) {
            ...
            $two_factor_code = trim($this->exts->fetchTwoFactorCode());
            if (!empty($two_factor_code) && trim($two_factor_code) != '') {
                if ($this->exts->exists($this->microsoft_remember_me_selector)) {
    ...
    private function checkFillExternalLogin()
    {
        ...
        if ($this->exts->urlContains('idaptive.app/login')) {
            ...
            if ($this->exts->querySelector('input[name="username"]') != null) {
                ...
                sleep(1);
                $this->exts->click_by_xdotool('#okta-signin-submit');
                sleep(7);
            }
            ...
            if ($this->exts->extract('[data-se="factor-password"] .infobox-error p') != '') {
                $this->exts->loginFailure(1);
            }
            sleep(10);
    ...
    private function checkConfirmMicrosoftButton()
    {
        ...
        $this->exts->waitTillPresent("//button[contains(text(), 'Skip setup')]");
        if ($this->exts->exists("//button[contains(text(), 'Skip setup')]")) {
            // Great job! Your security information has been successfully set up. Click "Done" to continue login.
            $this->exts->click_element("//button[contains(text(), 'Skip setup')]");
            sleep(10);
        }
    ...
    private function checkMicrosoftTwoFactorMethod()
    {
        ...
        if ($this->exts->exists('#idDiv_RemoteNGC_PollingDescription, #idRemoteNGC_DisplaySign, .confirmIdentityPageControl #iPollSessionDesc')) {
            ...
            $submit_selector = 'input#idSubmit_SAOTCS_SendCode, input#iSelectProofAction[type="submit"], input[type="submit"]';
            $this->exts->two_factor_attempts = 1;
            if ($this->microsoft_phone_number != '' && is_numeric(trim(substr($this->microsoft_phone_number, -1, 4)))) {
                $last4digit = substr($this->microsoft_phone_number, -1, 4);
...
";
    ...
    private function processAfterLogin($count)
    {
        ...
        $this->exts->waitTillPresent('a[data-name="switch_account_request"], a#nav-item-switch-account', 30);
        if ($this->exts->exists('a#nav-item-switch-account')) {
            $this->exts->openUrl($this->exts->querySelector('a#nav-item-switch-account')->getAttribute('href'));
            $this->exts->waitTillPresent('a[data-name="switch_account_request"]', 30);
        }
        $accounts = $this->exts->queryElementAll("a[data-name='switch_account_request']");

        for ($i = 0; $i < count($accounts); $i++) {
            $this->exts->waitTillPresent('a[data-name="switch_account_request"], a#nav-item-switch-account', 30);
            $this->exts->capture('Multi Accounts');
            if ($this->exts->exists('a#nav-item-switch-account')) {
                $this->exts->openUrl($this->exts->querySelector('a#nav-item-switch-account')->getAttribute('href'));
                $this->exts->waitTillPresent('a[data-name="switch_account_request"]', 30);
            }
            $this->exts->click_element($this->exts->querySelectorAll("a[data-name='switch_account_request']")[$i]);
            sleep(10);
            if (stripos($this->exts->getUrl(), "amazon.de/ap/signin") !== false) {
                $this->fillForm(0);
                $this->checkFillTwoFactor();
                sleep(10);
            }
            $this->exts->capture('Start Processing Account-' . $i);
            if ($count == 0) {
                if ($this->exts->exists('#nav-flyout-anchor a[href*="/order-history"]')) {
                    $this->exts->click_element('#nav-flyout-anchor a[href*="/order-history"]');
                } else if ($this->exts->exists('a[href*="/order-history"]')) {
            ...
            sleep(10);
            if ($this->checkLogin() && stripos($this->exts->getUrl(), "amazon.de/ap/signin") === false) {
                    ...
                    if ($isMultiAccount > 0) {
                        ...
                        if (count($selectAccountElements) > 0) {
                            foreach ($selectAccountElements as $selectAccountElement) {
                                if ((int) $this->only_business_invoice == 0) {
                                    if ((int) $this->procurment_report == 1) {
                                        $elementAccountValue = trim($selectAccountElement->getAttribute('value'));
                                        if (strtolower($elementAccountValue) == 'yourself') {
                                ...
                                } else if ((int) $this->only_business_invoice == 1 && (int) $this->procurment_report == 0) {
                                    $elementAccountValue = trim($selectAccountElement->getAttribute('value'));
                                    if (stripos($elementAccountValue, 'B2B') !== false) {
                                        $optionAccountSelectors[] = $elementAccountValue;
                                    }
                                }
                            }
                ...
                if ((empty($this->last_state['stage']) || $this->last_state['stage'] == 'PROCUREMENT_ANALYSIS') && ((int) $this->procurment_report == 1 || (int) $this->only_business_invoice == 0)) {
                    // Keep current state of processing
                    $this->current_state['stage'] = 'PAY_AS_INVOICE';
                    $this->last_state['stage'] = '';
                    ...
                    if ($this->exts->exists('a.nav-link[href*="/b2b/aba/"]')) {
                        ...
                        // Click Order report
                        if ($this->exts->exists('a.nav-link[href*="/b2b/aba/"]')) {
                            $selector = 'li.report-nav-item a[href*="/b2b/aba/reports?reportType=items_report_1"]';
                            $url = $this->exts->exists($selector) ? $this->exts->querySelector($selector)->getAttribute('href') : '';
                            if ($url != '' || $url != null) {
                                $this->exts->openUrl($url);
                            } else {
                                $this->exts->click_element($selector);
                            }
                        } else {
                ...
                //open account page
                //Check and download pay as invoice if available
                if (empty($this->last_state['stage']) || $this->last_state['stage'] == 'PAY_AS_INVOICE') {
                    // Keep current state of processing
                    $this->current_state['stage'] = 'PAY_AS_INVOICE';
                    $this->last_state['stage'] = '';

                    if ((int) @$this->pay_as_invoice == 1) {
                        if ($this->exts->exists('a[href*="ref_=bnav_youraccount_btn"]') || $this->exts->exists('a[href*="ref_=nav_youraccount_btn"]') || $this->exts->exists('a[href*="/gp/b2b/manage/overview"][href*="&ref_=nav_youraccount_bnav_ya_ad_manage"]') || $this->exts->exists('a[href*="ref_=abn_bnav_youraccount_btn"]')) {
                            if ($this->exts->exists('a[href*="ref_=bnav_youraccount_btn"]')) {
                                $this->exts->click_by_xdotool('a[href*="ref_=bnav_youraccount_btn"]');
                            } else if ($this->exts->exists('a[href*="/gp/b2b/manage/overview"][href*="&ref_=nav_youraccount_bnav_ya_ad_manage"]')) {
                                $this->exts->click_by_xdotool('a[href*="/gp/b2b/manage/overview"][href*="&ref_=nav_youraccount_bnav_ya_ad_manage"]');
                            } else if ($this->exts->exists('a[href*="ref_=abn_bnav_youraccount_btn"]')) {
                                $this->exts->click_by_xdotool('a[href*="ref_=abn_bnav_youraccount_btn"]');
                            } else {
                                $this->exts->click_by_xdotool('a[href*="ref_=nav_youraccount_btn"]');
                            }
                            sleep(10);
                            ...
                            //https://www.amazon.de/gp/b2b/manage/overview?ie=UTF8&ref_=nav_youraccount_bnav_ya_ad_manage
                            if (!$this->exts->exists('a[href*="ref_=ya_d_l_b2b_manage_business"]') && !$this->exts->exists('a[href*="/b2b/invoices?groupId="]') && $this->exts->exists('.nav-logo-business-prime')) {
                                $this->exts->openUrl('https://www.amazon.de/gp/b2b/manage/overview?ie=UTF8&ref_=nav_youraccount_bnav_ya_ad_manage');
                                sleep(5);
                            }
                            sleep(5);
                            if ($this->exts->exists('a.b-clickable[href*="ref_=ya_d_l_b2b_manage_business"]') || $this->exts->exists('a[href*="/b2b/invoices?groupId="]')) {
                                if ($this->exts->exists('a.b-clickable[href*="ref_=ya_d_l_b2b_manage_business"]')) {
                                    $this->exts->click_by_xdotool('a[href*="ref_=ya_d_l_b2b_manage_business"]');
                                    sleep(15);
                                }

                                //Open Pay by Invoice list
                                if ($this->exts->exists('a[href*="/b2b/invoices?groupId="]')) {
                                    $this->exts->click_by_xdotool('a[href*="/b2b/invoices?groupId="]');
                                    sleep(15);

                                    // Apply filter
                                    if ($this->exts->exists('awsui-multiselect[id="statement-box-status-multiselect"] div[role="button"]')) {
                                        $this->exts->click_element('awsui-multiselect[id="statement-box-status-multiselect"] div[role="button"]');
                                        sleep(5);
                                        $this->exts->click_element('//li[div[@title="Abgeschlossen"]]');
                                        sleep(2);

                                        if ($this->exts->exists('#statement-box-time-period-dropdown')) {
                                            $this->exts->click_element('#statement-box-time-period-dropdown');
                                            sleep(3);

                                            $this->exts->execute_javascript('
                                        let selectBox = document.querySelector("#statement-box-time-period-dropdown");
                                        selectBox.value = "LAST_12_MONTHS";
                                        selectBox.dispatchEvent(new Event("change"));
                                    ');
                                            sleep(3);
                                            $this->exts->click_by_xdotool('button[id="statement-box-search-button"]');
                                            sleep(15);
                                        }
                                    }

                                    $this->download_pay_invoice();
                                } else {
                                    //Wait and Try again to find link because sometime link load after full page load get completed
                                    sleep(15);
                                    if ($this->exts->exists('a[href*="/b2b/invoices?groupId="]')) {
                                        $this->exts->click_by_xdotool('a[href*="/b2b/invoices?groupId="]');
                                        sleep(2);
                                        ...
                                        if ($this->exts->exists('awsui-multiselect[id="statement-box-status-multiselect"] div[role="button"]')) {
                                            ...
                                            if ($this->exts->exists('#statement-box-time-period-dropdown')) {
                                                ...
                                                $this->exts->execute_javascript('
                                            const selectBox = document.getElementById("#statement-box-time-period-dropdown");
                                            selectBox.value = "LAST_12_MONTHS";
                                            selectBox.dispatchEvent(new Event("change"));
                                        ');
                                                sleep(3);
                                                $this->exts->click_by_xdotool('button[id="statement-box-search-button"]');
                                                sleep(15);
                                                $this->exts->execute_javascript("arguments[0].click();", [$this->exts->querySelector('button[id="statement-box-search-button"]')]);
                                            }
                                        }
                ...
                //https://www.amazon.de/businessprimeversand
    ...
    private function orderYearFilters($selectedBusinessAccount = "")
    {
        ...
        $selectElements = $this->exts->querySelectorAll("select[name='orderFilter'] option, select[name='timeFilterDropdown'] option");
    ...
    private function processYears($optionSelectors)
    {
        ...
        foreach ($optionSelectors as $optionSelector) {
            ...
            // Selecto box is hidden user see custom sropdown
            ...
            $this->exts->log("processing year element  " . $optionSelector);
            if ($this->exts->exists('span[data-a-class="order-filter-dropdown"] span[role="button"], div#orderTimeFilter span.a-button-dropdown, form.js-time-filter-form span.a-button-dropdown')) {
                $this->exts->click_element('span[data-a-class="order-filter-dropdown"] span[role="button"], div#orderTimeFilter span.a-button-dropdown, form.js-time-filter-form span.a-button-dropdown');
                sleep(3);

                $optionSelEle = 'a[id*="orderFilter"][data-value*="' . $optionSelector . '"], a[id*="timeFilterDropdown"][data-value*="' . $optionSelector . '"], a[id*="time-filter"][data-value*="' . $optionSelector . '"]';
                $selectElement = $this->exts->querySelector($optionSelEle);
                $this->exts->click_element($selectElement);
            }
            // else {
            //     $this->exts->changeSelectbox('select[name="timeFilter"]', $optionSelector);
            // }
            ...
            $this->exts->waitTillPresent("div.a-box-group.a-spacing-base.order, .order-card, #orderCard", 30);
            // $this->exts->capture("invoice pagination-".$optionSelector."-".$key1);
            if (!$this->exts->exists("div.a-box-group.a-spacing-base.order, .order-card, #orderCard")) {
                sleep(15);
            }
            $this->exts->capture('order-page');
            for ($paging_count = 1; $paging_count < 100; $paging_count++) {
                if ($this->exts->querySelector(".order-card, .js-order-card, div#yourOrderInfoSection #orderCard") != null) {
                    $this->exts->log("Invoice Found");

                    $invoice_data_arr = array();
                    $rows = $this->exts->querySelectorAll(".order-card, .js-order-card, div#yourOrderInfoSection #orderCard");
                    $this->exts->log("Invoice Rows- " . count($rows));
                    $total_rows = count($rows);
                    $orderFilteredUrl = $this->exts->getUrl();
                    if (count($rows) > 0) {
                        for ($i = 0, $j = 2; $i < $total_rows; $i++, $j++) {
                            $rows = $this->exts->querySelectorAll(".order-card, .js-order-card, div#yourOrderInfoSection #orderCard");
                            $rowItem = $rows[$i] ?? null;
                            if ($rowItem == null) {
                                $this->exts->openUrl($orderFilteredUrl);
                                sleep(20);
                                $rows = $this->exts->querySelectorAll(".order-card, .js-order-card, div#yourOrderInfoSection #orderCard");
                                $rowItem = $rows[$i] ?? null;
                            }
                            ...
                            try {
                                $columns = $rowItem->querySelectorAll('div.order-info div.a-fixed-right-grid-col:nth-child(1) span.a-color-secondary.value, .order-header .a-row:nth-child(2) .a-color-secondary,  #orderCardHeader .a-row:nth-child(2)');
                                ...
                                if (count($columns) > 0) {
                                    ...
                                    $columns = $rowItem->querySelectorAll('div.order-info div.a-fixed-right-grid-col:nth-child(1) span.a-color-secondary.value, .order-header .a-row:nth-child(2) .a-color-secondary,  #orderCardHeader .a-row:nth-child(2)');
                                    $invoice_number = $this->exts->extract('[class*="order-id"] .a-color-secondary:nth-child(2), li#orderIdField', $rowItem, 'innerText');
                                    ...
                                    if (count($columns) > 0) {
                                        ...
                                        if (stripos($detailPageUrl, "https://www.amazon.de") === false && stripos($detailPageUrl, "https://") === false) {
                                            $detailPageUrl = "https://www.amazon.de" . trim($detailPageUrl);
                                        }
                                        ...
                                        if (trim($detailPageUrl) != "" && $this->dateLimitReached == 0) {
                                            ...
                                            if ($last_date_timestamp > strtotime($invoice_date) && $this->last_invoice_date != "" && !empty($this->last_invoice_date) && trim($parsed_date) != "") {
                                                ...
                                                if (count($isPopOver) > 0) {
                                                    ...
                                                    $links = $this->exts->querySelectorAll('.a-popover[aria-hidden="false"] .invoice-list a');
                                                ...
                                                if (!empty($invoice_urls)) {
                                                    ...
                                                    foreach ($invoice_urls as $invoice_url_item) {
                                                        ...
                                                        if (trim($invoice_url) != "") {
                                                            ...
                                                            if (stripos($invoice_url, "https://www.amazon.de") === false && stripos($invoice_url, "https://") === false) {
                                                                $invoice_url = "https://www.amazon.de" . $invoice_url;
                                                            }
                                                            $this->exts->log("invoice_url - " . $invoice_url);

                                                            if (trim($overview_link) != "" && stripos($overview_link, "https://www.amazon.de") === false && stripos($overview_link, "https://") === false) {
                                                                $overview_link = "https://www.amazon.de" . $overview_link;
                                                            }
                                                ...
                                                if (!empty($savedInvoices)) {
                                                    ...
                                                    foreach ($savedInvoices as $savedInvoice) {
                                                        if (stripos($savedInvoice['invoice_url'], "print") !== false) {
                                                            ...
                                                            if (trim($contact_url) != "" && stripos($contact_url, "https://www.amazon.de") === false) {
                                                                $contact_url = "https://www.amazon.de" . $contact_url;
                                                            }
                            ...
                            $this->exts->log('Invoice Downloaded For Row ' . $i);
                            if (stripos($this->exts->getUrl(), "/your-account/order-history") === false) {
                                $this->exts->log('Opening Order Page');
                                $this->exts->switchToInitTab();
                                sleep(2);
                                $this->exts->openUrl($orderFilteredUrl);
                                sleep(15);
                ...
                if ($this->exts->exists('.a-pagination li.a-selected + li:not(.a-disabled) a')) {
                    ...
                    $this->exts->click_element('.a-pagination li.a-selected + li:not(.a-disabled) a');
                    sleep(10);
                } else {
                    break;
                }
    ...
    private function getTotalYearPages($reloadCount)
    {
        $pages = 0;
    ...
    private function downloadBusinessPrimeInvoices()
    {
        $this->exts->update_process_lock();
        ...
        if ($this->exts->querySelector("a[href*=\"/bb/benefits/bps/landing/?ref_=ab_bps_engmt_mgmtbox\"]") != null) {
            ...
            if ($this->exts->querySelector("a#business-prime-shipping-view-last-invoice") != null) {
                try {
                    ...
                    if (trim($invoice_url) != "" && stripos($invoice_url, "https://www.amazon.de") === false && stripos($invoice_url, "https://") === false) {
                        $invoice_url = "https://www.amazon.de" . $invoice_url;
                    }
                    $this->exts->log("prime invoice URL - " . $invoice_url);
                } catch (\Exception $exception) {
            ...
            $this->exts->click_element('a[href*="/businessprime/manage/"], .membership-settings-cta a[href*="/businessprime/manage"], section.management a[href*="/businessprime/manage"]');
            sleep(15);

            // Added invoices row check as after the above button click user is redirecting directly to the invoices page and no further clicks are needed
            if (!$this->exts->exists('.a-box .a-row, table tbody tr')) {
                ...
                foreach ($rows as $row) {
                    if ($this->exts->querySelector('a[href*="/documents/download/"][href*="/Invoice.pdf"]', $row) != null) {
                        ...
                        if (count($cols) > 4) {
                            ...
                            if (!empty($parse_date) && $parse_date != null)
    ...
    private function downloadPrimeInvoice()
    {
        $this->exts->update_process_lock();
        //Check if user is having private prime invoices
        $this->exts->openUrl('https://www.amazon.de/mc?_encoding=UTF8&ref_=ya_d_c_prime');
        sleep(5);
        ...
        $this->exts->waitTillPresent('a[href*="/mc/receipt?transaction="], a[href*="order-summary"]');
        $this->exts->capture("private-prime");
        if ($this->exts->exists('a[href*="/mc/receipt?transaction="], a[href*="order-summary"]')) {
            $invoices = $this->exts->querySelectorAll('a[href*="/mc/receipt?transaction="], a[href*="order-summary"]');
            $invoice_url = [];
            ...
            foreach ($invoice_url as $url) {
                ...
                if (stripos($this->exts->getUrl(), "amazon.de/ap/signin") === false && stripos($this->exts->getUrl(), "print=1") !== false) {
                    $currentUrl = $this->exts->getUrl();
    ...
    private function triggerMsgInvoice()
    {
    ...
    private function processMSInvoice($currentMessagePage)
    {
        $this->exts->update_process_lock();
            ...
            foreach ($invMsgRows as $key => $invMsgRow) {
                ...
                if (count($invMsgColImg) > 0 && count($invMsgCols) > 0) {
                    ...
                    if (empty($invMsgColImgTitle))
                        $invMsgColImgTitle = trim($invMsgColImg[0]->getAttribute("title"));
    ...
    private function startCurrentPageMessageDownloadNew()
    {
        ...
        for ($i = 1; $i <= count($rows); $i++) {
            ...
            $this->exts->waitTillPresent('div.view-order a', 30);
            $this->exts->click_element('div.view-order a');
    ...
    private function startCurrentPageMessageDownload($inv_msgs)
    {
        foreach ($inv_msgs as $inv_msg) {
            ...
            if (trim($inv_msg['msg_thread_id']) != "") {
                $msgUrl = "https://www.amazon.de/gp/message?ref_=ya_d_l_msg_center#!/detail/" . $inv_msg['msg_id'] . "/" . $inv_msg['msg_thread_id'] . "/bsm/" . $inv_msg['msg_time'] . "/inbox";
            } else {
                $msgUrl = "https://www.amazon.de/gp/message?ref_=ya_d_l_msg_center#!/detail/" . $inv_msg['msg_id'] . "/bsm/" . $inv_msg['msg_time'] . "/inbox";
            }
    ...
    private function download_pay_invoice($count = 0, $page = 1)
    {
        $this->exts->update_process_lock();
        $this->exts->log("Start downloading business Pay Invoice");
        $receipts = $this->exts->querySelectorAll('div.invoicing-tab-invoices a[href*="/b2b/invoices/details?"], table tbody tr.b-clickable');
        $total_receipts = count($receipts);
        $this->exts->log('Total Receipts Found - ' . $total_receipts);
        if ($total_receipts > 0) {
            foreach ($receipts as $receipt) {
                $invoice_name = $this->exts->querySelector('div.invoicing-statement span.invoicing-id-col, div[id="transaction-box-statement-document-number"]  div', $receipt)->getText();
                $this->exts->log("Invoice Name: " . $invoice_name);

                $invoice_date = $this->exts->querySelector('div.invoicing-statement span.invoicing-date-col, td:nth-child(2)', $receipt)->getText();
                $this->exts->log("Invoice Date: " . $invoice_date);
                try {
                    $parsed_date = $this->exts->parse_date($invoice_date);
                } catch (\Exception $exception) {
                ...
                $invoice_amount = $this->exts->querySelector('div.invoicing-statement span.invoicing-amount-col, td:nth-child(5)', $receipt)->getText();
                $this->exts->log("Invoice Amount: " . $invoice_amount);
                ...
                if ($invoice_url != '') {
                    ...
                    if ($this->exts->exists('a.a-button-text[href*="/documents/download/"]')) {
                        $download_url = $this->exts->querySelector('a.a-button-text[href*="/documents/download/"]')->getAttribute("href");
                    } else {
                        if (count($this->exts->querySelectorAll('a[href*="/documents/download/"]')) > 0) {

                            $download_url = $this->exts->querySelector('a[href*="/documents/download/"]')->getAttribute("href");
                        }
                    }

                    if (trim($download_url) != "") {
                        if (trim($download_url) != "" && stripos($download_url, "https://www.amazon.de") === false && stripos($download_url, "https://") === false) {
                            $download_url = "https://www.amazon.de" . $download_url;
                        }
                        $filename = trim($invoice_name) . ".pdf";
            ...
            if ($this->restrictPages == 0 && $this->exts->exists('ul.a-pagination li.a-last a[href*="/b2b/invoices?groupId="]')) {
                $this->exts->click_by_xdotool('ul.a-pagination li.a-last a[href*="/b2b/invoices?groupId="]');
                sleep(10);
                $page++;
    ...
    private function invoice_overview_exists($invoice_number)
    {
        $this->exts->update_process_lock();
    ...
    private function download_procurment_document($pageNum = 1)
    {
        $this->exts->update_process_lock();
        ...
        do {
            if ($pageNum > 1) {
                if ((int) $this->restrictPages == 0) {
                    if ($this->exts->querySelector('.report-table-footer button[data-testid="next-button"]') != null && $this->exts->querySelector('.report-table-footer button[data-testid="next-button"][disabled]') == null) {
                    ...
                    if ($this->exts->querySelector('.report-table-footer button[data-testid="next-button"]') != null && $this->exts->querySelector('.report-table-footer button[data-testid="next-button"][disabled]')  == null && $pageNum < 50) {
                        $this->exts->click_by_xdotool('.report-table-footer button[data-testid="next-button"]');
                        sleep(15);
                        $pageNum++;
                    } else {
                        ...
                        foreach ($downloadLinks as $downloadLink) {
                            if (stripos($downloadLink, '/b2b/aba/order-summary/') !== false) {
                                if (trim($orderNum) !== '' && !$this->exts->invoice_exists($orderNum) && !$this->invoice_overview_exists($orderNum)) {
                                    ...
                                    if (stripos($this->exts->getUrl(), "amazon.de/ap/signin") === false && stripos($this->exts->getUrl(), "/print.html") !== false) {
