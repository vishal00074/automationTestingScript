...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 2141139 - Last modified: 09.09.2025 11:22:58 UTC - User: 1

    public $baseUrl = 'https://app.funnelish.com/';
    public $loginUrl = 'https://app.funnelish.com/log-in';
    public $invoicePageUrl = 'https://app.funnelish.com/settings/billing';

    public $username_selector = 'input[autocomplete="email"]';
    public $password_selector = 'input[autocomplete="password"]';
    public $remember_me_selector = '';
    public $submit_login_selector = 'a.login-btn';
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            //select account
            sleep(5);
            $accountItems = $this->exts->getElements('div[class="entries-table panel-body"] > div');
            if ($accountItems) {
                $totalItems = count($accountItems);
                for ($i = 0; $i < $totalItems; $i++) {
                    $this->exts->click_element($this->exts->getElements('div[class="entries-table panel-body"] > div')[$i]);
                    sleep(8);
                    $this->exts->openUrl($this->invoicePageUrl);
                    $this->downloadInvoices();
                    sleep(3);
                    $this->exts->openUrl('https://app.funnelish.com/select-account');
                }
            } else {
                sleep(5);
                $this->exts->openUrl($this->invoicePageUrl);
            ...
            if (stripos($error_text, strtolower('Incorrect login or password')) !== false) {
    ...
    private function downloadInvoices()
    {
        ...
        sleep(3);
        $this->waitForSelectors("table tbody tr", 5, 5);
