<?php // added code to close popup and updated invoice history button selector  updated load more invoices button code
// and adjust the sleep time according to ui 
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 1361509 - Last modified: 19.08.2025 10:58:43 UTC - User: 1

    public $baseUrl = 'https://app.neuroflash.com/login';
    public $loginUrl = 'https://app.neuroflash.com/login';
    public $invoicePageUrl = 'https://app.neuroflash.com/profile/billing';

    public $username_selector = 'input[name="email"], input[id="loginName"]';
    public $password_selector = 'input[name="password"]';
    public $remember_me_selector = '';
    public $submit_login_selector = 'button[data-testid="login-button"], button[id="submit-button"]';

    public $check_login_failed_selector = 'div.MuiAlert-message, p.lgn-error-message';
    public $check_login_success_selector = 'div[data-gtm-interaction="GTM_sidebar_accountMenu"]';
    ...
    private function initPortal($count)
    {
        ...
        if (!$this->checkLogin()) {
			...
			var accept_cookies_wrapper = document.querySelector("#usercentrics-cmp-ui");
		    if(accept_cookies_wrapper){
		        let shadow = accept_cookies_wrapper.shadowRoot;
		        if(shadow){
		            let button = shadow.querySelector("button#accept");
		            if(button) button.click()
		        }
		    }
	    ');
            $this->exts->moveToElementAndClick('button[name="userID"][value="0"]');
            sleep(3);
            $this->fillForm(0);
            sleep(7);
            $this->exts->execute_javascript('
		...
		');
            sleep(7);
            $this->exts->execute_javascript('let skip_button = document.querySelector("#trial-modal-maybe-later-button"); if(skip_button) skip_button.click();');
            sleep(7);
        }
        ...
        ');
            ...
            sleep(7);
            $this->exts->click_element('.ReactModalPortal #trial-modal-maybe-later-button');

            if ($this->exts->queryElement('div#interactive-close-button-container') != null) {
                $this->exts->moveToElementAndClick('div#interactive-close-button-container');
                sleep(5);
            }

            $this->exts->openUrl($this->invoicePageUrl);
            sleep(7);
            $this->exts->click_element(".//h6[text()='Invoice history' or text()='Zu den Rechnungen']/following-sibling::a");
            sleep(5);
    ...
    private function fillForm($count)
    {
        ...
        try {
            if ($this->exts->querySelector($this->username_selector) != null) {
                ...
                sleep(5);
                if ($this->exts->urlContains('accounts.google')) {
