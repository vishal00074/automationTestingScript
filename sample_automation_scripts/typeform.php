<?php // handle empty invoiceName exists to isExists added restrict page logic
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 7298 - Last modified: 31.07.2025 12:47:24 UTC - User: 1
    ...
    private function invoicePage()
    {
        ...
        if (!$this->isExists('a[data-qa="link-to-flow"][href*="/accounts/"]') && !$this->isExists('button[data-qa="organization-avatar-rebrand"]')) {
            $this->exts->openUrl('https://admin.typeform.com/user');
            sleep(10);
        }
        // Click admin setting
        $this->exts->moveToElementAndClick('a[data-qa="link-to-flow"][href*="/accounts/"], button[data-qa="organization-avatar-rebrand"]');
        sleep(10);
        $this->exts->moveToElementAndClick('a[data-qa="sidebar-account-plan-billing"], a[href*="plan"]');
        sleep(15);

        $this->downloadInvoice();

        if ($this->totalFiles == 0 && $this->isNoInvoice) {
    ...
    private function downloadInvoice()
    {
        ...
        $this->exts->moveToElementAndClick('button[data-qa="toggle-expand-invoices-button"]');
        sleep(7);
        $restrictPages = isset($this->exts->config_array["restrictPages"]) ? (int)@$this->exts->config_array["restrictPages"] : 3;
        $this->exts->capture('4-List-invoice');
        if ($this->isExists('[data-qa="plan-and-billing-container"] [data-qa="invoices-table-card"] table tbody tr td')) {
            $rows = count($this->exts->getElements('[data-qa="plan-and-billing-container"] [data-qa="invoices-table-card"] table tbody tr'));
            for ($i = 0; $i < $rows; $i++) {
                ...
                $row = $this->exts->getElements('[data-qa="plan-and-billing-container"] [data-qa="invoices-table-card"] table tbody tr')[$i];
            ...
            // Some invoices are link with url, some only have button to click
            $rows = count($this->exts->getElements('[data-qa="plan-and-billing-container"] li'));
            for ($i = 0; $i < $rows; $i++) {
                ...
                $row = $this->exts->getElements('[data-qa="plan-and-billing-container"] li')[$i];
                if ($this->exts->getElement('div a[href*="/billing/invoice/"]', $row) != null) {
                    $this->totalFiles++;
                    $invoice_url = $this->exts->extract('div a[href*="/billing/invoice/"]', $row, 'href');
                    $temp_array = explode('billing/invoice/', $invoice_url);
                    $invoice_name = end($temp_array);
                    $temp_array = explode('/', $invoice_name);
                    $invoice_name = reset($temp_array);
                    $invoice_filename = !empty($invoice_name) ? $invoice_name . '.pdf' : '';
                    ...
                    sleep(10);
                    //This condition is added because sometime page show error and that get print as invoice.
                    if ($this->isExists('#content .companies, [class*="invoice-details__StyledWrapper"]')) {
                        $downloaded_file = $this->exts->download_current($invoice_filename, 5);
                        if (trim($downloaded_file) != "" && file_exists($downloaded_file)) {
                            $this->exts->new_invoice($invoice_name, '', '', $downloaded_file);
                        }
                    } else {
                        $this->exts->capture('invoice_detail_error-' . $i);
                    }
                    $this->exts->switchToInitTab();
                    $this->exts->closeAllTabsButThis();
                } else if ($this->exts->getElement('a[href*="/invoices/"]', $row) != null) {
                    $this->totalFiles++;
                    $invoice_url = $this->exts->extract('a[href*="/invoices/"]', $row, 'href');
                    $temp_array = explode('/invoices/', $invoice_url);
                    $invoice_name = end($temp_array);
                    $temp_array = explode('/', $invoice_name);
                    $invoice_name = reset($temp_array);
                    $invoice_filename = !empty($invoice_name) ? $invoice_name . '.pdf' : '';
                    ...
                    //This condition is added because sometime page show error and that get print as invoice.
                    if ($this->isExists('#content .companies, [class*="invoice-details__StyledWrapper"]')) {
                        $downloaded_file = $this->exts->download_current($invoice_filename, 1);
                        if (trim($downloaded_file) != "" && file_exists($downloaded_file)) {
                            $this->exts->new_invoice($invoice_name, '', '', $downloaded_file);
                            $this->totalInvoices++;
                        }
                    ...
                    $this->exts->switchToInitTab();
                    $this->exts->closeAllTabsButThis();
                } else if ($this->exts->getElement('[orientation="horizontal"] > div:last-child button [aria-label*="Download"]', $row) != null) {
                    $this->totalFiles++;
                    $download_button = $this->exts->getElement('[orientation="horizontal"] > div:last-child button', $row);
                    $this->exts->moveToElementAndClick($download_button);
                    sleep(5);
                    $handles = $this->exts->get_all_tabs();
                    if (count($handles) > 1) {
                        $this->exts->switchToTab(end($handles));
                        sleep(5);
                        $invoice_url = $this->exts->getUrl();
                        if (stripos($invoice_url, '/invoices/') !== false) {
                            $temp_array = explode('/invoices/', $invoice_url);
                            $invoice_name = end($temp_array);
                            $temp_array = explode('/', $invoice_name);
                            $invoice_name = reset($temp_array);
                        ...
                        if (count($handles) > 1) {
                            $this->exts->switchToTab(end($handles));
                            $this->exts->closeTab(end($handles));
                            $handles = $this->exts->get_all_tabs();
