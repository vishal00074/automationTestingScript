...
// Server-Portal-ID: 10252 - Last modified: 20.01.2025 06:37:15 UTC - User: 1
...
if(count($selectElements) == 0) {
    $selectElements = $this->exts->exists('select[name="timeFilter"] option');
    $this->exts->log("selectElements ".count($selectElements));
...
foreach($optionSelectors as $optionSelector) {
    ...
    if(!$this->exts->exists(".order-card, .js-order-card")) {
        sleep(15);
    }
    ...
    for ($paging_count=1; $paging_count < 100; $paging_count++) { 
        if($this->exts->getElement(".order-card, .js-order-card") != null) {
            ...
            $rows = $this->exts->exists(".order-card, .js-order-card");
            ...
            if(count($rows) > 0) {
                for($i=0, $j=2; $i<$total_rows; $i++,$j++) {
                    $rowItem = $rows[$i];
                    try {
                        $columns = $rowItem->getElements('div.order-info div.a-fixed-right-grid-col:nth-child(1) span.a-color-secondary.value, .order-header .a-row:nth-child(2) .a-color-secondary');
                        ...
                        if(count($columns) > 0) {
                            ...
                            $sellerName = "";
                            $sellerColumns = $rowItem->getElements("div.a-box.shipment div.a-fixed-left-grid-col.a-col-right span.a-color-secondary");
                            if(count($sellerColumns) > 0) {
                                ...
                                if(trim($sellerName) != "" && stripos(trim($sellerName), ": Amazon EU S.a.r.L.") !== false && count($sellerColumns) > 1) {
                                    $sellerColumns1 = $rowItem->getElements("div.a-box.shipment div.a-fixed-left-grid-col.a-col-right span.a-size-small.a-color-secondary");
                            ...
                            $detailPageUrl = "";
                            $columns = $rowItem->getElements('div.order-info div.a-fixed-right-grid-col.actions ul a.a-link-normal, a[href*="/order-details"]');
                            if(count($columns) > 0) {
                                ...
                                if(trim($detailPageUrl) != "" && $this->dateLimitReached == 0) {
                                    ...
                                    if($last_date_timestamp > strtotime($invoice_date) && $this->last_invoice_date != "" && !empty($this->last_invoice_date) && trim($parsed_date) != "") {
                                        ...
                                        $prices = array();
                                        $price_blocks = $rowItem->getElements("div.a-box.shipment div.a-fixed-left-grid-col.a-col-right span.a-color-price");
                                        ...
                                        $isPopOver = $rowItem->getElements('a[href*="/ajax/invoice/"], [data-a-popover*="invoice"] a');
                                        ...
                                        if(count($isPopOver) > 0) {
                                            ...
                                            $links = $rowItem->getElements("div.orderSummary a.a-link-normal");
                                            if(count($links) > 0) {
                                                $invoice_urls[] = array(
                                            ...
                                            foreach($invoice_urls as $invoice_url_item) {
                                                ...
                                                if(trim($invoice_url) != "") {
                                                    if(stripos($invoice_url, "contact") !== false) {
                                                        $links = $rowItem->getElements(".a-popover-content a.a-link-normal");
...
if($this->exts->getElement("span.num-orders-for-orders-by-date span.num-orders") != null) {
    ...
    if($pages < 0) {
        $pageEle = $this->exts->exists("div.pagination-full a");
...
$inv_msgs = array();
$invMsgRows = $this->exts->exists("//*[@id=\"inbox_bsm_tab_content\"]/tbody/tr");
...
if(count($invMsgRows) > 0) {
    ...
    foreach($invMsgRows as $key => $invMsgRow) {
        $mj = $key+1;
        $invMsgCols = $this->exts->exists("//*[@id=\"inbox_bsm_tab_content\"]/tbody/tr[$mj]/td");
        $invMsgColImg = $this->exts->exists("//*[@id=\"inbox_bsm_tab_content\"]/tbody/tr[$mj]//span/img");
...
foreach($inv_msgs as $inv_msg) {
    ...
    if($this->exts->getElement('.a-ordered-list a.a-link-normal') != null) {
        $links = $this->exts->exists(".a-ordered-list a.a-link-normal");
        foreach($links as $link_item) {
            $invoice_data = array();
            ...
            if(stripos($invoice_name, ".pdf") !== false) {
                ...
                $ordItems = $this->exts->exists("div#detail-page .a-box-inner a[href*=\"summary/edit.html\"]");
                if(count($ordItems) > 0) {
