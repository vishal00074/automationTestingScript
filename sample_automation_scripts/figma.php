...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 77708 - Last modified: 25.04.2025 13:17:29 UTC - User: 1
    ...
    public $login_confirm_selector = "div[data-testid='ProfileButton']";
    ...
    private function initPortal($count)
    {
        ...
        if (!$isCookieLoginSuccess) {
            ...
            if ((int) @$this->login_with_google == 1) {
                ...
                $this->fillForm(0);
                sleep(10);
                if ($this->isExists('div[class*="onboarding_components"] div[class*="close_button--closeX"]')) {
            ...
            if ($this->checkLogin()) {
                ...
                if ($this->exts->extract('#auth-view-page div[class*="auth_form--error"]') != '') {
                    ...
                    $this->exts->loginFailure(1);
                } elseif (stripos($this->exts->extract('p[class*="auth_form_2024_brand_updates--error"]'), 'That email and password combination is incorrect.') !== false) {
                    $this->exts->capture("LoginFailed");
                    $this->exts->loginFailure(1);
                } else {
                    $this->exts->capture("LoginFailed");
    ...
    private function fillForm($count)
    {
        ...
        try {
            $this->waitForSelectors($this->username_selector, 5, 3);
            if ($this->exts->getElement($this->username_selector) != null) {
                ...
                sleep(5);
                if ($this->isExists($this->login_button_selector) && !$this->isExists($this->password_selector)) {
                ...
                if ($this->isExists($this->password_selector)) {
                    ...
                    if ($this->isExists($this->login_button_selector)) {
                        $this->exts->moveToElementAndClick($this->login_button_selector);
                        sleep(5);
                    }
                }
    ...
    private function fillFormOnelogin($count = 0)
    {
        $this->exts->log("Begin fillForm " . $count);
        try {
            if ($this->exts->getElement('input#username') != null) {
                ...
                sleep(5);
                $this->exts->moveToElementAndClick('button[type="submit"]');
                $this->waitForSelectors("[type='error']", 3, 3);
                if ($this->isExists('[type="error"]')) {
    ...
    private function checkFillLogin_undetected_mode($root_user_agent = '')
    {
        ...
        if ($this->exts->check_exist_by_chromedevtool($this->google_password_selector)) {
            ...
            $continueButton = $this->exts->getElements('button[type="button"]');

            if (count($continueButton) > 0) {
                $this->exts->click_by_xdotool($continueButton[0]);
                sleep(5);
    ...
    private function checkLogin()
    {
        ...
        $isLoggedIn = false;
        try {
            $this->waitForSelectors("div[data-testid='ProfileButton'], input[name='email']", 10, 5);
            if ($this->exts->getElement($this->login_confirm_selector) != null && !$this->isExists($this->username_selector)) {
                $this->exts->log(">>>>>>>>>>>>>>>Login successful!!!!");
                $isLoggedIn = true;
            }
        } catch (\Exception $exception) {
    ...
    private function ProcessMultilTeams()
    {
        sleep(25);
        $this->exts->capture("process_multil_teams");
        // Click Dropdown Button
        $this->waitForSelectors("div[data-onboarding-key='PLAN_SWITCHER_ONBOARDING_KEY'] button[class*='dropdown_button']", 5, 4);
        $this->exts->click_element("div[data-onboarding-key='PLAN_SWITCHER_ONBOARDING_KEY'] button[class*='dropdown_button']");
        sleep(2);
        if ($this->isExists('div[class*=planSwitcherRow]')) {
            $team_len = count($this->exts->querySelectorAll('div[class*=planSwitcherRow]'));
            $this->exts->log('Total Team length : ' . $team_len);
            if ($team_len > 0) {
                for ($i = 0; $i < $team_len; $i++) {
                    sleep(3);
                    $team_sel = $this->exts->getElements('div[class*=planSwitcherRow]')[$i];
                    if ($team_sel) {
                        $this->exts->executeSafeScript("arguments[0].click()", [$team_sel]);
                        sleep(25);

                        if ($this->isExists('div[data-testid="admin-settings-link"]')) {
                            $this->exts->click_element('div[data-testid="admin-settings-link"]');
                            sleep(15);
                            ...
                            // $this->waitForSelectors("div[class*='mostRecentInvoiceContainer'] button", 5, 5);
                            // $this->exts->moveToElementAndClick('div[class*="mostRecentInvoiceContainer"] button');
                            $this->waitForSelectors('div[role="tablist"] button[id*="invoices-trigger"]', 5, 5);
                            $this->exts->moveToElementAndClick('div[role="tablist"] button[id*="invoices-trigger"]');
                            sleep(5);
                        ...
                        sleep(15);
                        $this->exts->click_element('div[data-onboarding-key="PLAN_SWITCHER_ONBOARDING_KEY"] button');
                    } else {
    ...
    {
        $rows = $this->exts->querySelectorAll('div[class*="adminTableRow"]');
        $invoices = [];
        foreach ($rows as $key => $row) {
            ...
            $invoiceUrl = '';
            $invoiceDate = $this->exts->extract('div[class*="plan_invoices_table--dateColumn"] span', $row);
            $invoiceAmount = $this->exts->extract('div[class*="plan_invoices_table--dateColumn"] span', $row);

            $row->click();
            $this->waitForSelectors("a[href*= 'invoice.stripe']", 5, 3);
            $download_link = $this->exts->getElement('a[href*= "invoice.stripe"]');
            if ($download_link) {
                $invoiceUrl = $download_link->getAttribute("href");
            }
            $invoiceName = $this->exts->extract('a[href*= "invoice.stripe"]');
        ...
        foreach ($invoices as $invoice) {
            ...
            $this->waitForSelectors("div.InvoiceDetailsRow-Container button:nth-child(1)", 5, 3);
