...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 15579 - Last modified: 21.07.2025 14:27:34 UTC - User: 1

    public $baseUrl = 'https://www.flaticon.com/profile/my_subscriptions';
    public $username_selector = 'input#gr_login_username, input#login-username, #log-in input[name="email"]';
    public $password_selector = 'input#gr_login_password, input#login-password, #log-in input[name="password"]';
    public $remember_me_selector = 'input[name="keep-signed"]';
    public $submit_login_selector = '#signin_button, button[id*="auth-login-form"], #log-in button[type="submit"]';
    public $check_login_failed_selector = '#auth-login-form .error-message, form p.error';
    public $check_login_success_selector = '#logout_link';
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->querySelector($this->check_login_success_selector) == null) {
            ...
            if ((int) $this->login_with_google == 1) {
                $this->exts->moveToElementAndClick('#log-in button.googlelogin');
                sleep(10);
                ...
                $tab_buttons = $this->exts->getElements('div.continue-with button.main-button');
                ...
                foreach ($tab_buttons as $key => $tab_button) {
                    ...
                    if (stripos($tab_name, 'Continue with email') !== false) {
        ...
        if ($this->exts->querySelector($this->check_login_success_selector) != null) {
            ...
            $invoice_history_link_selector = 'a#modal-invoicing-link';
            $this->exts->moveToElementAndClick($invoice_history_link_selector);
            sleep(10);
    ...
    private function checkFillLogin()
    {
        if ($this->exts->querySelector($this->password_selector) != null) {
            ...
            $this->exts->waitTillPresent($this->submit_login_selector, 30);
    ...
    private function checkFillRecaptcha($try_count = 1)
    {
    ...
    {
        $two_factor_selector = '.verification-code input';
        $two_factor_message_selector = 'p.code-sent';
        $two_factor_submit_selector = 'button.main-button';
    ...
    private function processInvoices()
    {
        ...
        foreach ($rows as $row) {
            ...
            if (count($tags) >= 5 && $this->exts->querySelector('a[href*="/downloadinvoice"]', $tags[5]) != null) {
                $invoiceUrl = $this->exts->querySelector('a[href*="/downloadinvoice"]', $tags[5])->getAttribute("href");
