<?php // i have updated check_login_success selector and increase sleep time to checking login success added custom waitFor function replace waittillPresent
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 163759 - Last modified: 05.03.2025 14:28:23 UTC - User: 1
    ...
    function cleanInvoiceAmount($value = '')
    {
    ...
    {
        $this->waitFor('#payment-billing ul li', 10);
        if ($this->exts->exists('div.banner button.btn-success')) {
            $this->exts->moveToElementAndClick('div.banner button.btn-success');
            sleep(1);
        }
        ...
        $maxBackDate = date('Y-m-d', strtotime('-1 year'));
        if ($this->restrictPages == 0) {
            $maxBackDate = date('Y-m-d', strtotime('-3 year'));
        }
        $this->exts->log($maxBackDate);
        $ul_selector = '#payment-billing ul li';
        $this->waitFor('#payment-billing ul li', 10);
        $this->exts->capture('1-invoices page');
        $invoices = [];
        if ($this->exts->exists($ul_selector)) {

            $lis = $this->exts->querySelectorAll($ul_selector);
            $this->exts->log('Invoice Table Rows Found : ' . count($lis));
            foreach ($lis as $key => $li) {

                $invoice_date = $this->exts->querySelector('strong', $li)->getAttribute('innerText');
                $invoice_amount = $this->cleanInvoiceAmount(trim(explode(":", $li->getAttribute('innerText'))[1])) . ' EUR';
                $invoice_url = $this->exts->querySelector('a', $li)->getAttribute('href');

                $invoice_number = explode("/", $invoice_url)[4];
