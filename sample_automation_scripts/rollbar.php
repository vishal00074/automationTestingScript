...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 8004 - Last modified: 20.01.2025 06:20:36 UTC - User: 1

    /*Define constants used in script*/
    public $baseUrl = "https://rollbar.com/login/";
    public $username_selector = "input[placeholder*='username'], [name='username_or_email']";
    public $password_selector = "input[placeholder*='password'], [name='password']";
    public $submit_btn = "button[type=submit]";
    public $logout_btn = "[href='/logout/'] , #NavBar, form[action*='/logout']";
    public $wrong_credential_selector = "form#login-form .form-error,.alert-danger.form-error";
    ...
    private function initPortal($count)
    {
        ...
        if ($this->isExists('div#nav-account-profile')) {
            $this->exts->moveToElementAndClick('div#nav-account-profile');
            sleep(5);
        }

        if (!$this->checkLogin() && !$this->isExists($this->wrong_credential_selector)) {
            ...
            for ($i = 0; $i < 8; $i++) {
                ...
                if ($this->exts->getElement($this->username_selector) != null &&  $this->exts->getElement($this->wrong_credential_selector) == null) {
                ...
                if ($this->exts->getElement($this->logout_btn) !== null || $this->exts->getElement($this->wrong_credential_selector) !== null) {
                    break;
                }
            }
            ...
            if ($this->isExists('[name="twofa_token"]')) {
                $this->processTwoFactorAuth('[name="twofa_token"]', '#login-form [type=submit]');
                sleep(2);
            }
            if ($this->exts->urlContains('google.')) {
                $this->loginGoogleIfRequired();
            }
            $this->exts->moveToElementAndClick('div#NavBar a img[src*="/avatar"]');
            sleep(5);
            if ($this->isExists('div#nav-account-profile')) {
                $this->exts->moveToElementAndClick('div#nav-account-profile');
                sleep(5);
        ...
        if ($this->checkLogin()) {
            ...
            $this->exts->capture("LoginFailed" . $this->exts->getUrl());
            $mes = strtolower($this->exts->extract('div.signup-box div.signup-form ~ div.form-info', null, 'innerText'));
            if ($this->isExists($this->wrong_credential_selector)) {
                $this->exts->loginFailure(1);
            } else if ($this->isExists('div.signup-box div.signup-form') && strpos($mes, 'want to create a personal account') !== false) {
    ...
    private function checkFillRecaptcha($counter)
    {
        ...
        if ($this->isExists($recaptcha_iframe_selector)) {
			...
			recurse(___grecaptcha_cfg.clients[2], "", 0);
			return found ? "___grecaptcha_cfg.clients[2]." + result : null;
		');
    ...
    function checkLogin()
    {
        ...
        try {
            ...
            if ($this->isExists('a#nav-dashboard')) {
                $this->exts->log(">>>>>>>>>>>>>>>Login successful!!!!");
                $isLoggedIn = true;
            }
        } catch (\Exception $exception) {
    ...
    public function downloadInvoice($count)
    {
        sleep(25);
        ...
        $rows = $this->exts->getElements('.invoices-table > tbody > tr');
        foreach ($rows as $row) {
            $tags = $this->exts->getElements('td', $row);
            if (count($tags) >= 5 && $this->exts->getElement('a[href*="/invoices/"]', $tags[4]) != null) {
                $invoiceUrl = $this->exts->getElement('a[href*="/invoices/"]', $tags[4])->getAttribute("href");
                $invoiceName = explode(
                    '/',
