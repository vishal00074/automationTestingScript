...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 874681 - Last modified: 24.02.2025 13:10:38 UTC - User: 1
    ...
    {
        sleep(3);
        $this->waitFor('div.account--orders-overview div.js--collapse-target', 15);
        $this->exts->capture("4-invoices-page");
        $invoices = [];

        $rows = $this->exts->querySelectorAll('div.account--orders-overview div.js--collapse-target');
        foreach ($rows as $row) {
            if ($this->exts->querySelector('a.order--document-link[href*="document/download"]', $row) != null) {
                $invoiceUrl = $this->exts->querySelector('a.order--document-link[href*="document/download"]', $row)->getAttribute('href');
                $invoiceName = $this->exts->extract('a.order--document-link[href*="document/download"]', $row);
                $invoiceName = trim(str_replace("\n", ' ', $invoiceName));
                $invoiceAmount =  trim($this->exts->extract('div.column--summary-data p:last-child', $row));
                preg_match('/\((\d{2}\.\s\w+\s\d{4})\)/', $this->exts->extract('a.order--document-link', $row), $matches);
                $invoiceDate =  $matches[1];;

                $downloadBtn = $this->exts->querySelector('a.order--document-link', $row);
        ...
        if (
            $restrictPages == 0 &&
            $paging_count < 50 &&
            $this->exts->querySelector("a.paging--next") != null
        ) {
            $paging_count++;
            $this->exts->log('Next invoice page found');
            $this->exts->click_element("a.paging--next");
            sleep(5);
