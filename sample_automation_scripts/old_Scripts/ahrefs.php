<?php // handle empty invoiceName case updated loginfailed selector updated two selector 
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 1241 - Last modified: 10.07.2025 14:55:55 UTC - User: 1
    ...
    private function processInvoices()
    {
        ...
        $invoices = [];
        $this->exts->waitTillPresent('div[class*="tabsContent"] table tbody tr');
        $rows = $this->exts->querySelectorAll('div[class*="tabsContent"] table tbody tr');
        foreach ($rows as $index => $row) {
            $tags = $this->exts->querySelectorAll('td', $row);
            if (count($tags) >= 6 && $this->exts->querySelector('a', $tags[4]) != null) {
                $invoiceUrl = $this->exts->querySelector('a', $tags[4])->getAttribute("href");
                $this->exts->log('Invoice URL - ' . $invoiceUrl);
                if (stripos($invoiceUrl, '/invoice') !== false) {
                    $tempInvUrl = trim($invoiceUrl, '/pdf');
                    $invoiceName = array_pop(explode('invoice/', $tempInvUrl));
                    $invoiceDate = str_replace("'", "", trim($tags[2]->getAttribute('innerText')));
            ...
            } else if (count($tags) >= 6 && $this->exts->querySelector('a', $tags[5]) != null) {
                $invoiceUrl = $this->exts->querySelector('a', $tags[5])->getAttribute("href");
                $this->exts->log('Invoice URL - ' . $invoiceUrl);
                if (stripos($invoiceUrl, '/invoice') !== false) {
                    $tempInvUrl = trim($invoiceUrl, '/pdf');
                    $invoiceName = array_pop(explode('invoice/', $tempInvUrl));
                    $invoiceDate = str_replace("'", "", trim($tags[3]->getAttribute('innerText')));
                    ...
                    $this->isNoInvoice = false;
                } else if (stripos($invoiceUrl, '/download-invoice/') !== false) {
                    $tempInvUrl = trim($invoiceUrl, '/pdf');
                    $invoiceNameTemp = explode('/download-invoice/', $tempInvUrl);
                    $invoiceName = end($invoiceNameTemp);
                    $invoiceDate = str_replace("'", "", trim($tags[3]->getAttribute('innerText')));
                    $invoiceAmount = trim(preg_replace('/[^\d\.\,]/', '', $tags[4]->getAttribute('innerText'))) . ' USD';
        ...
        foreach ($invoices as $invoice) {
            ...
            $invoice['invoiceDate'] = $this->exts->parse_date($invoice['invoiceDate'], 'd F Y', 'Y-m-d', 'en');
