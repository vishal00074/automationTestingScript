<?php //  handle empty invoices cases fix undefined $row varaibel error and added time after click_and_download function
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 110069 - Last modified: 29.07.2025 15:00:15 UTC - User: 1

    // Start Script

    public $baseUrl = 'https://biz.alibaba.com/order/list.htm';
    public $loginUrl = 'https://login.alibaba.com/';
    public $invoicePageUrl = 'https://biz.alibaba.com/order/list.htm';
    public $username_selector = 'input#fm-login-id, input[name="account"]';
    public $password_selector = 'input#fm-login-password, input[name="password"]';
    public $remember_me_selector = '';
    public $submit_login_selector = 'input#fm-login-submit, button[class="sif_form-submit"]';
    public $check_login_failed_selector = 'div#login-error a';
    // public $check_login_success_selector = 'a[href*="/order/list.htm?"] , //a[text()[contains(., "Log out")]] , a[href*="signout"], .tnh-loggedin a[href*="message.alibaba.com"]';
    public $check_login_success_selector = '.menu-list-box a[href*="/order/list.htm?"]';
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->querySelector($this->check_login_success_selector) == null) {
            ...
            if ($this->login_with_google == 1) {
                $this->exts->moveToElementAndClick('.login-login-links a.icon-google, div[id="sif_third-party-login"] a.icon-google');
                sleep(3);
                ...
                if ($this->exts->exists('iframe[src*="mini_login_check"], iframe[src*="token_enter"]')) {
            ...
            if ($this->exts->exists('.gdpr-agree-btn')) {
                $this->exts->moveToElementAndClick('.gdpr-agree-btn');
                sleep(3);
            }
        }
        sleep(5);
        ...
        if ($this->exts->querySelector($this->check_login_success_selector)) {
    ...
    private function checkFillLogin()
    {
        if ($this->exts->getElement($this->password_selector) != null) {
            ...
            sleep(7);
            // after enter username, slide captcha may showed.
    ...
    {
        $two_factor_iframe = $this->exts->makeFrameExecutable('iframe[src*="mini_login_check"], iframe[src*="token_enter"]');
        sleep(40);
        $two_factor_iframe->moveToElementAndClick('div[class="baxia-dialog-close"]');
        sleep(10);
        $two_factor_selector = '.ui-input-checkcode, .au_input';
        $two_factor_message_selector = 'div.ui-tiptext-message, .ui-form-item .ui-form-text, div.au_title';
        $two_factor_submit_selector = 'button[type="submit"], button.au_submit';
        $alt_two_factor_submit_selector = 'button#btn-submit';

        if ($two_factor_iframe->getElement($two_factor_selector) != null && $this->exts->two_factor_attempts < 3) {
            $this->exts->log("Two factor page found.");
            $this->exts->capture("2.1-two-factor");

            if ($two_factor_iframe->getElement($two_factor_message_selector) != null) {
                $this->exts->two_factor_notif_msg_en = "";
                for ($i = 0; $i < count($two_factor_iframe->getElements($two_factor_message_selector)); $i++) {
                    $this->exts->two_factor_notif_msg_en = $this->exts->two_factor_notif_msg_en . $two_factor_iframe->getElements($two_factor_message_selector)[$i]->getAttribute('innerText') . "\n";
                }
            ...
            if (!empty($two_factor_code) && trim($two_factor_code) != '') {
                ...
                $two_factor_iframe->moveToElementAndType($two_factor_selector, $two_factor_code);

                $this->exts->log("checkFillTwoFactor: Clicking submit button.");
                sleep(3);
                if ($two_factor_iframe->getElement($two_factor_submit_selector) != null) {
                    $two_factor_iframe->moveToElementAndClick($two_factor_submit_selector);
                } else {
                    $two_factor_iframe->moveToElementAndClick($alt_two_factor_submit_selector);
                }
                sleep(15);

                if ($two_factor_iframe->getElement($two_factor_selector) == null) {
    ...
    private function processInvoicesOld($paging_count = 1)
    {
        sleep(5);
        $this->exts->update_process_lock();
        $this->exts->moveToElementAndClick('div.gdpr-agree-btn');
        sleep(10);
        if ($this->exts->exists('a.notice-dialog-close')) {
            $this->exts->moveToElementAndClick('a.notice-dialog-close');
            sleep(5);
        }
        ...
        for ($paging_count = 1; $paging_count < 100; $paging_count++) {
            $invoices = [];
            $order_links = $this->exts->getElements('div.order-list-item-body-right a[href*="detail"][href*="orderId"]');
            foreach ($order_links as $order_link) {
                // $invoiceUrl = $order_link->getAttribute("href");
                $invoiceUrl = $this->exts->execute_javascript('return arguments[0].href', [$order_link]);
                $temp_array = explode('orderId=', $invoiceUrl);
            ...
            foreach ($invoices as $invoice) {
                ...
                if ($this->exts->invoice_exists($invoice['invoiceName'])) {
                    ...
                    sleep(20);
                    $this->exts->waitTillPresent('[data-i18n-key*="paymentRecord"]');
                    $this->exts->moveToElementAndClick('[data-i18n-key*="paymentRecord"]');
                    sleep(3);
                    $invoice_buttons = $this->exts->getElements('div[id*="PaymentRecord"] span[data-i18n-key*="action_invoce"]');

                    $this->exts->log('----------- invoice_buttons--------------- ' . count($invoice_buttons));
                    if (count($invoice_buttons) == 1) {
                        $this->exts->moveToElementAndClick('div[id*="PaymentRecord"] span[data-i18n-key*="action_invoce"]');
                        sleep(3);
                        if ($this->exts->config_array['browser'] == 'optimized-chrome') {
                            $this->exts->waitTillPresent('a[href*="feeInvoice"]:last-child', 10);
                            if ($this->exts->exists('a[href*="feeInvoice"]:last-child')) {
                                $hiden_download = $this->exts->getElement('a[href*="feeInvoice"]:last-child');
                                $pdf_url = $this->exts->execute_javascript('return arguments[0].href;', [$hiden_download]);
                                $this->exts->log('----------- pdf_url--------------- ' . $pdf_url);
                                $this->exts->openUrl($pdf_url);
                                sleep(15);
                            }
                        }
                    ...
                    } else if (count($invoice_buttons) > 1) {
                        for ($i = 0; $i < count($invoice_buttons); $i++) {
                            $invoice_button = $this->exts->getElements('div[id*="PaymentRecord"] span[data-i18n-key*="action_invoce"]')[$i];
                            try {
                            ...
                            if (trim($downloaded_file) != '' && file_exists($downloaded_file)) {
                                ...
                                $this->exts->new_invoice($invoice_name, $invoice['invoiceDate'], $invoice['invoiceAmount'], $downloaded_file);
                                sleep(20);
                            } else {
            ...
            if ($this->exts->exists('.order-list-pagination button.next-next:not([disabled])')) {
                if ($this->exts->config_array['restrictPages'] == '0' || $paging_count < 5) {
                    $this->exts->moveToElementAndClick('.order-list-pagination button.next-next:not([disabled])');
                    sleep(10);
                } else {
                    break;
                }
    ...
    private function processInvoices($paging_count = 1)
    {
        ...
        $this->exts->capture("4-invoices-page");
        $invoices = [];
        $order_links = $this->exts->querySelectorAll('div.order-list-item');

        $this->exts->log('No of Order Details Count :  ' . count($order_links));

        foreach ($order_links as $row) {
            $orderNumber = $this->exts->extract('div[class="order-list-item-header"] div.order-info > span:nth-child(2)', $row);
            preg_match('/#(\d+)/', $orderNumber, $matches);
            $invoiceName = $matches[1];

            $orderDate = $this->exts->extract('div[class="order-list-item-header"] div.order-info > span:nth-child(3)', $row);
            preg_match('/\d{2} \w{3} \d{4}/', $orderDate, $matches);
            $invoiceDate = $matches[0];

            $invoiceAmount = $this->exts->extract('div[class="order-list-item-header"] div.order-info > span:nth-child(4) > span:nth-child(2)', $row);
            ...
            $viewOrderDetails = $this->exts->querySelector(' div.order-list-item-body > div:nth-child(2) a', $row);
            if ($viewOrderDetails) {
                $this->exts->click_element($viewOrderDetails);
                sleep(10);
            }
            ...
            $downloadBtn = $this->exts->getElement('div.download-action');
        ...
        if (
            $restrictPages == 0 &&
            $paging_count < 50 &&
            $this->exts->querySelector('.order-list-pagination button.next-next:not([disabled])') != null
        ) {
            $paging_count++;
