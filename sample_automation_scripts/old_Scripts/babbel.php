...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 9588 - Last modified: 09.10.2024 13:07:00 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->getElement($this->check_login_success_selector) != null) {
            ...
            sleep(15);
            $this->exts->log('invoice url:' . $this->exts->getUrl());
            if ($this->exts->exists('div.up-subscriptions__list')) {
                $this->processInvoices();
            } else {
                $this->processInvoicesPayments();
            }
    ...
    private function processInvoices()
    {
        ...
        $rows = $this->exts->getElements('a[href*=".invoices."]');
        foreach ($rows as $row) {
            $invoiceUrl = $row->getAttribute("href");
            $invoiceName = trim($this->exts->getElement('span.up-subscription__date', $row)->getAttribute("innerText"));
            $invoiceDate = trim($this->exts->getElement('span.up-subscription__date', $row)->getAttribute("innerText"));
            $invoiceAmount = '';
    ...
    private function processInvoicesPayments()
    {
        sleep(25);
        ...
        $rows = $this->exts->getElements('ul.invoices > li');
        foreach ($rows as $row) {
            if ($this->exts->getElement('a[href*=".pdf"]', $row) != null) {
                $invoiceUrl = $this->exts->getElement('a[href*=".pdf"]', $row)->getAttribute("href");
                $invoiceName = trim(end(explode('babbel.com', explode('?', $invoiceUrl)[0])));
                $invoiceDate = trim($row->getAttribute('innerText'));
        ...
        $rows = $this->exts->getElements('a[href*=".pdf"][href*=".invoices."]');
        foreach ($rows as $row) {
            $invoiceUrl = $row->getAttribute("href");
            $invoiceName = trim(str_replace('.', '', $row->getAttribute('innerText')));
            $invoiceDate = trim($row->getAttribute('innerText'));
            $invoiceDate = trim(end(explode(' vom ', $invoiceDate)));
            $invoiceAmount = '';
        ...
        foreach ($invoices as $invoice) {
            ...
            if ($date_parsed == '') {
                $date_parsed = $this->exts->parse_date($invoice['invoiceDate'], 'j. M. Y', 'Y-m-d');
            }
