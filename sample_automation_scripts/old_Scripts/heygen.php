<?php // handle empty invoice name updated navigate to invoices selector replace waitTillPresent to custom js waitFor
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 2027037 - Last modified: 31.07.2025 00:08:21 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            sleep(30);
            // $this->waitFor('//div[text()="Manage billing info"]', 30);
            if ($this->exts->exists('.//div[text()="Manage billing info"]')) {
                $this->exts->click_element('.//div[text()="Manage billing info"]');
            } else if ($this->exts->exists('.//div[contains(text(),"Subscriptions") and contains(text(),"API")]')) {
                $this->exts->moveToElementAndClick('.//div[contains(text(),"Subscriptions") and contains(text(),"API")]');
                sleep(10);
                if ($this->exts->exists('.//button[text()="Manage Billing"]')) {
                    $this->exts->moveToElementAndClick('.//button[text()="Manage Billing"]');
                }
            }
    ...
    {
        sleep(10);
        $this->waitFor('a[href*="invoice.stripe.com"]', 20);
        ...
        foreach ($invoices as $invoice) {
            ...
            sleep(8);
            if ($this->exts->getElement('//button[contains(@class, "Button--primary")]//span[contains(text(),"Rechnung herunterladen") or contains(text(),"Download invoice")]') != null) {
