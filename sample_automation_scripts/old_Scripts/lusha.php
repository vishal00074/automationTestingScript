<?php // replace exists to isExists function handle empty invoiceName added code to close survey box added disable extension agfter login success getting issue while opening invoice url
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 213126 - Last modified: 14.07.2025 15:04:38 UTC - User: 1
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->getElement($this->check_login_success_selector) != null) {
            ...
            $this->exts->moveToElementAndClick('div[role=dialog]>button');
            sleep(2);
            // Open invoices url and download invoice
            $this->exts->openUrl($this->invoicePageUrl);
            sleep(15);
            if ($this->isExists('div#survicate-box button.sv__btn-close')) {
                $this->exts->moveToElementAndClick('div#survicate-box button.sv__btn-close');
                sleep(1);
            }
    ...
    ");
        $this->exts->capture('disable-extension');
    }
    ...
    private function processInvoices($paging_count = 1)
    {
        ...
        for ($i = 0; $i < $rows; $i++) {
            ...
            if (count($tags) >= 4 && $this->exts->getElement('button[data-test-id="menu-action-undefined"]', $tags[3]) != null) {
                $this->isNoInvoice = false;
                $button_view = $this->exts->getElement('button[data-test-id="menu-action-undefined"]', $tags[3]);
                ...
                if ($this->exts->invoice_exists($invoiceName)) {
                    ...
                    $button_view->click();
                    sleep(2);
                    $this->exts->moveToElementAndClick('//li//span[contains(text(),"View Invoice")]');
                    sleep(5);
        ...
        if (
            ...
            $paging_count < 50 &&
            $this->exts->getElement('button[data-test-id="next-page-btn-undefined"]:not([disabled])') != null
        ) {
            $this->exts->moveToElementAndClick('button[data-test-id="next-page-btn-undefined"]:not([disabled])');
            sleep(5);
