<?php // updated invoice name and invoice amount selector added code to trigger loginfailedConfirmed in case incorrect credentials
/**
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 37095 - Last modified: 14.02.2025 06:29:28 UTC - User: 15

    /*Define constants used in script*/
    public $baseUrl = 'https://next.frame.io/settings/billing';
    public $username_selector = 'input[name="email"]';
    public $password_selector = 'input#password, input#PasswordPage-PasswordField';
    public $submit_login_selector = 'button[type="submit"], button[data-id="PasswordPage-ContinueButton"]';

    public $check_login_failed_selector = 'form div[class*="ErrorMessage"]';
    public $check_login_success_selector = 'a[class^="DashboardSidebar"][href*="accounts"], .fio-navbar-account-popover-trigger, div[data-testid="account-avatar"]';
    ...
    private function initPortal($count)
    {
        ...
        if ($this->exts->querySelector($this->check_login_success_selector) == null) {
            ...
            $error_text = strtolower($this->exts->extract('span.InlineError__content'));
            ...
            if ($this->exts->exists('button[name="google-auth"]') || $this->exts->urlContains('google.com')) {
                $this->loginGoogleIfRequired();
            }

            if (stripos($this->exts->extract('div[data-id="Page-Description"]'), 'send you a verification code') !== false && stripos($this->exts->extract('button[data-id="Page-PrimaryButton"]'), 'continue') !== false) {
        ...
        for ($i = 0; $i < 10; $i++) {
            if ($this->exts->querySelector($this->check_login_success_selector) != null || $this->exts->getElement('//button[contains(text(), "Explore V4")]', null, 'xpath') != null || $this->exts->getElement('//button[contains(text(), "Go back to V3")]', null, 'xpath') != null) {
                $user_logged_in = true;
                break;
            } else {
                sleep(3);
        ...
        if ($this->exts->getElement('//button[contains(text(), "Explore V4")]', null, 'xpath') != null) {
            $this->exts->click_element('//button[contains(text(), "Explore V4")]');
            sleep(30);
        }

        if ($this->exts->getElement('//button[contains(text(), "Go back to V3")]', null, 'xpath') != null) {
            $this->exts->click_element('//button[contains(text(), "Go back to V3")]');
            sleep(30);
        }
        ...
        if ($user_logged_in) {
            ...
            if ($this->isNoInvoice) {
                $this->exts->openUrl('https://app.frame.io/account/billing?k=billing&t=invoices');
                $this->processInvoices();
            }
            ...
            if (stripos($this->exts->extract('h2[class*="ModalHeader__Title"]'), 'Please confirm your email') !== false) {
                $this->exts->account_not_ready();
            }
            if ($this->exts->querySelector($this->check_login_failed_selector) != null) {
                $this->exts->loginFailure(1);
            } else if ($this->exts->exists('div[class*="SignUp__PasswordFieldContainer"]') && $this->exts->urlContains('signup')) {
                $this->exts->loginFailure(1);
            } else {
    ...
    {
        $this->exts->capture("2-frameio-login-page");
        ...
        if ($this->exts->querySelector($this->password_selector) != null) {
            ...
            $this->exts->capture("2-frameio-login-page-filled");
            $this->exts->click_by_xdotool($this->submit_login_selector);
        } else {
            $this->exts->log(__FUNCTION__ . '::frameio password not found');
            $this->exts->capture("2-frameio-password-page-not-found");
        }
    }
    private function checkFillTwoFactor()
    {
        $two_factor_selector = 'input[data-id*="CodeInput-"]';
        ...
        if ($this->exts->querySelector($two_factor_selector) != null && $this->exts->two_factor_attempts < 3) {
            ...
            if (!empty($two_factor_code) && trim($two_factor_code) != '') {
                $otp_digits = str_split($two_factor_code);
                foreach ($otp_digits as $index => $digit) {
                    $selector = $two_factor_selector . '[data-index="' . $index . '"]';
                    $this->exts->log("checkFillTwoFactor: Entering digit " . $digit . " into field " . $index);
                    $this->exts->moveToElementAndType($selector, $digit);
                }
    ...
    private function processInvoices()
    {
        ...
        foreach ($rows as $row) {
            ...
            if (count($tags) >= 4 && $this->exts->querySelector('a[href*="/invoice/"]', $tags[3]) != null) {
                ...
                $invoiceUrl = trim($invoiceUrl, '/') . '/pdf';
            ...
            } else if (count($tags) >= 4 && $this->exts->querySelector('a[href*="invoice.stripe.com"]', $tags[3]) != null) {
                $invoiceUrl = $this->exts->querySelector('a[href*="invoice.stripe.com"]', $tags[3])->getAttribute("href");
                $invoiceName = trim($tags[3]->getText());
                $invoiceDate = trim($tags[0]->getText());
                $invoiceAmount = trim(preg_replace('/[^\d\.\,]/', '', $tags[1]->getText())) . ' USD';
        ...
        foreach ($invoices as $invoice) {
            ...
            if (stripos($invoice['invoiceUrl'], 'invoice.stripe.com') !== false) {

                $this->exts->openUrl($invoice['invoiceUrl']);
                sleep(10);

                $this->exts->click_by_xdotool('.InvoiceDetails-table button:nth-child(1)');
                sleep(25);
                ...
                if (trim($downloaded_file) != '' && file_exists($downloaded_file)) {
                    ...
                    $this->exts->new_invoice($invoice_name, $invoice['invoiceDate'], $invoice['invoiceAmount'], $invoiceFileName);
                    sleep(1);
                } else {
