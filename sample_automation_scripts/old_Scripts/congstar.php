<?php //  handle empty invoice name case updated two_factor_submit_selector added code to trigger loginFailedConfirmed in case twoFA fail
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 4904 - Last modified: 11.07.2025 13:33:59 UTC - User: 15
    ...
    private function processInvoices()
    {
        ...
        $invoices = [];
        $rows = $this->exts->getElements('[data-test-row-bill-number]');
        $this->exts->log("_________Rows: " . count($rows));
        foreach ($rows as $row) {
            if ($this->exts->getElement('div.row[data-test-row-bill-number] a[href*="customer-bills/pdf/"][data-test-action-bill-download]', $row) != null) {
                $invoiceUrl = $this->exts->extract('div.row[data-test-row-bill-number] a[href*="customer-bills/pdf/"][data-test-action-bill-download]', $row, 'href');
                $invoiceName =  end(explode('pdf/', $invoiceUrl));
                $invoiceDate = trim($this->exts->extract('div.row[data-test-row-bill-number] span[data-test-bill-date]', $row));
                $invoiceDate = trim(str_replace(')', '', str_replace('(', '', $invoiceDate)));
                $invoiceAmount = trim($this->exts->extract('div.row[data-test-row-bill-number] span[data-test-bill-amount]', $row));
        ...
        foreach ($invoices as $invoice) {
            ...
            $invoiceDate = $this->exts->parse_date($invoice['invoiceDate']);
            if ($invoiceDate == '') {
            ...
            if (trim($downloaded_file) != '' && file_exists($downloaded_file)) {
                $this->exts->new_invoice($invoice['invoiceName'], $invoiceDate, $invoice['invoiceAmount'], $downloaded_file);
                sleep(1);
            } else {
