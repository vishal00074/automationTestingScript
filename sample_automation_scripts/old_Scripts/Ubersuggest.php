...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 976554 - Last modified: 13.11.2024 13:09:20 UTC - User: 1

    /*Define constants used in script*/

    public $baseUrl = 'https://app.neilpatel.com/en/login';
    public $loginUrl = 'https://app.neilpatel.com/en/login';
    public $invoicePageUrl = 'https://app.neilpatel.com/en/settings/account_billing';

    public $username_selector = 'input[name="email"]';
    public $password_selector = 'input[name="password"]';
    public $remember_me_selector = '[data-testid="remember-login-checkbox"]';
    public $submit_login_selector = 'button[data-testid="login-button"]';

    public $check_login_failed_selector = 'div[data-testid="login-error-message"]';
    public $check_login_success_selector = 'div[data-testid="nav-logo-button"]';
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            sleep(2);
            if ($this->exts->exists('button[data-testid="dismiss-button"]')) {
                $this->exts->click_by_xdotool('button[data-testid="dismiss-button"]');
            }
            ...
            sleep(10);
            $this->exts->click_element("//button[contains(text(), 'Download Invoice')]");
            sleep(10);
            $this->exts->switchToNewestActiveTab();
            sleep(4);
            $invoicePage = $this->exts->getUrl();
            $invoicePageUrl =  $invoicePage . '/invoices';
    ...
    {
        $this->exts->waitTillPresent('table#invoices tbody tr', 20);
        ...
        foreach ($rows as $row) {
            if ($this->exts->querySelector('td:nth-child(6) a', $row) != null) {
                $invoiceUrl = $this->exts->querySelector('td:nth-child(6) a', $row)->getAttribute('href');
        ...
        $pagiantionSelector = 'a.next_page';
        if ($restrictPages == 0) {
