<?php // replace exists and waitTillPresent to wait and isExists custom js function handle empty invoice name added disable_extensions
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 323 - Last modified: 21.07.2025 14:34:00 UTC - User: 1

    /*Define constants used in script*/
    public $baseUrl = "https://www.browserstack.com/users/sign_in";
    public $loginUrl = "https://www.browserstack.com/users/sign_in";
    public $homePageUrl = "https://www.browserstack.com/accounts/billing";
    public $username_selector = "input#user_email_login";
    public $password_selector = "div.toggle-auth-form:not(.hide) input#user_password";
    public $submit_button_selector = 'form#signin_signup_form input#user_submit[type="submit"]';
    ...
    private function initPortal($count)
    {
        ...
        if (!$isCookieLoginSuccess) {
            ...
            if ($this->isExists('#challenge-form #cf-hcaptcha-container iframe[src*="hcaptcha.com/captcha/"]')) {
                ...
                sleep(5);
                //after slove hcaptcha, open login page
                $this->exts->openUrl($this->loginUrl);
            }
            $this->fillForm(0);
            sleep(10);
            // if($this->isExists('form#enterprise_sign_in a[href*="google_oauth2"]') && !$this->isExists($this->password_selector)){
            // 	$this->exts->click_by_xdotool('form#enterprise_sign_in a[href*="google_oauth2"]');
            // 	sleep(10);
            // }
            // if ($this->exts->urlContains('google.com') && !$this->checkLogin()) {
            //     $this->exts->account_not_ready();
            // }

            if ($this->isExists('input#user_full_name:not(:disabled)') && $this->isExists($this->username_selector) && $this->isExists($this->password_selector)) {
                $this->exts->log("Wrong credentials!!");
                $this->exts->loginFailure(1);
            }

            $err_msg = $this->exts->extract('input#user_password + div.error-msg div.msg-body');
    ...
    private function fillForm($count)
    {
        ...
        try {
            ...
            if ($this->isExists($this->username_selector) || $this->isExists($this->password_selector)) {
                ...
                if ($this->isExists('div.google-login-button a')) {
                    $this->exts->click_by_xdotool('div.google-login-button a');
                    sleep(3);
...
";
    ...
    private function solve_captcha_by_clicking($count = 1)
    {
        ...
        $language_code = '';
        $unsolved_hcaptcha_submit_selector = '#challenge-form #cf-hcaptcha-container iframe[src*="hcaptcha.com/captcha/"]';
        $hcaptcha_challenger_wraper_selector = '#challenge-form #cf-hcaptcha-container iframe[src*="hcaptcha.com/captcha/"]';
    ...
    private function checkLogin()
    {
        ...
        $isLoggedIn = false;
        try {
            $this->waitFor('ul.primary-menu li ul#account-menu-dropdown li:last-child a.sign_out_link, button[aria-label="account popover"]', 20);
            if ($this->isExists('ul.primary-menu li ul#account-menu-dropdown li:last-child a.sign_out_link, button[aria-label="account popover"]') && !$this->isExists($this->password_selector)) {
                $this->exts->log(">>>>>>>>>>>>>>>Login successful!!!!");
                $isLoggedIn = true;
            }
        } catch (Exception $exception) {
    ...
    function invoicePage()
    {
        ...
        $this->exts->openUrl('https://www.browserstack.com/accounts/billing');
        $this->waitFor('a#get-all-invoices');

        if ($this->isExists('a#get-all-invoices')) {
            $this->exts->click_by_xdotool('a#get-all-invoices');
            sleep(3);
        }
    ...
    {
        $this->waitFor('div#all-invoices div.invoice-item, div.invoice-list table tbody tr, .invoice-table table tbody tr');
        $this->exts->log("Begin download invoice");
        $this->exts->capture('4-List-invoice');

        try {
            if ($this->exts->querySelector('div#all-invoices div.invoice-item') != null) {
                $receipts = $this->exts->querySelectorAll('div#all-invoices div.invoice-item');
                $invoices = array();
                foreach ($receipts as $receipt) {
                    if ($this->exts->querySelector('a[href*="Invoice-"]', $receipt) != null) {
                        $receiptDate = $this->exts->extract('div.invc-expiry', $receipt);
                        $receiptUrl = $this->exts->extract('a[href*="Invoice-"]', $receipt, 'href');
                        if (strpos($receiptUrl, 'name=') !== false) {
                            $receiptName = trim(explode('&', end(explode('name=', $receiptUrl)))[0]);
                        } else {
                            $receiptName = trim(explode('.pdf', end(explode('/', $receiptUrl)))[0]);
                        }
                        $receiptFileName = $receiptName . '.pdf';
                        $parsed_date = $this->exts->parse_date($receiptDate, 'dS M, Y', 'Y-m-d');
                        $receiptAmount = '';
            ...
            } else if ($this->isExists('div.invoice-list table tbody tr')) {
                $this->exts->click_by_xdotool('button.invoice-list__load-more');
                sleep(8);
                $booking_urls = [];
                ...
                for ($paging_count = 0; ($paging_count < $maxAttempts && $this->isExists('button.invoice-list__load-more')); $paging_count++) {
                    $this->exts->click_by_xdotool('button.invoice-list__load-more');
                    sleep(8);
                }
                ...
                $rows = $this->exts->querySelectorAll('div.invoice-list table tbody tr');
                foreach ($rows as $row) {
                    $tags = $this->exts->querySelectorAll('td', $row);
                    if (count($tags) >= 5 && $this->exts->querySelector('a[href*="get-invoice-pdf?id="]', $tags[4]) != null) {
                        $invoiceUrl = $this->exts->querySelector('a[href*="get-invoice-pdf?id="]', $tags[4])->getAttribute("href");
                        $invoiceName = explode(
                            '&',
                ...
                foreach ($invoices as $invoice) {
                    ...
                    $invoice['invoiceDate'] = $this->exts->parse_date($invoice['invoiceDate'], 'jS M, Y', 'Y-m-d');
            ...
            } else if ($this->isExists('.invoice-table table tbody tr')) {
                $this->exts->capture("4-invoices-page");
                $invoices = [];

                $rows = $this->exts->querySelectorAll('.invoice-table table tbody tr');
                foreach ($rows as $row) {
                    if ($this->exts->querySelector('td:nth-child(5) button[aria-label="Invoice"]', $row) != null) {
                        $invoiceUrl = '';
                        ...
                        $downloadBtn = $this->exts->querySelector('td:nth-child(5) button[aria-label="Invoice"]', $row);
                ...
                if (
                    ...
                    $this->exts->querySelector('button[aria-label="Next Page"][aria-disabled="false"]') != null
                ) {
                    $paging_count++;
                    $this->exts->click_by_xdotool('button[aria-label="Next Page"][aria-disabled="false"]');
                    sleep(5);
                    $this->downloadInvoice($paging_count);
                } else if ($this->restrictPages > 0 && $paging_count < $this->restrictPages && $this->exts->querySelector('button[aria-label="Next Page"][aria-disabled="false"]') != null && $this->totalFiles <= 50) {
                    $paging_count++;
                    $this->exts->click_by_xdotool('button[aria-label="Next Page"][aria-disabled="false"]');
                    sleep(5);
                    $this->downloadInvoice($paging_count);
