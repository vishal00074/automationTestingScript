<?php //elopage: update download code
// Server-Portal-ID: 17443 - Last modified: 27.01.2025 14:50:41 UTC - User: 1

// Script here
public $baseUrl = 'https://tesg.com/cabinet';
		...
		for ($a=0; $a < $total_accounts; $a++) {
			...
			if($account_disabled == null && $selecting_account != null){
				...
				try{
					...
					$this->exts->executeSafeScript("arguments[0].click()", [$selecting_account]);
				}
				sleep(7);
	...
	for ($i=0; $i < $rows; $i++) {
		...
		if($action_button != null) {
			...
			try{
				...
				$this->exts->executeSafeScript("arguments[0].click()", [$action_button]);
			}
			sleep(2);
	...
	for ($p=0; $p < $count_products; $p++) { 
		...
		if($order_manage_button != null){
			...
			try{
				...
				$this->exts->executeSafeScript("arguments[0].click()", [$order_manage_button]);
			}
