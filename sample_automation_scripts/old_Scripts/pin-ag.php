<?php // migrated script and updated remote chrome function added conditionin invoice name is empty
/**
...
class PortalScriptCDP
{
    ...
    private function processInvoices()
    {
        ...
        if ($restrictPages == 0 && $this->exts->getElement('button.load_more') != null) {
            $this->exts->moveToElementAndClick('button.load_more');
            sleep(10);
        }
        $this->exts->capture("4-invoices-page");
        $invoices = [];
        $rows = $this->exts->getElements('div>div>div.row');
        foreach ($rows as $row) {
            $tags = $this->exts->getElements('div.cell', $row);
            if (count($tags) >= 3 && $this->exts->getElement('a', $tags[2]) != null) {
                $invoiceUrl = $this->exts->getElement('a', $tags[2])->getAttribute("href");
                $invoiceName = explode(
                    ' vom',
                    array_pop(
                        explode('Rechnung ', trim($tags[0]->getText()))
                    )
                )[0];
                $invoiceDate = explode('vom ', trim($tags[0]->getText()))[1];
                $invoiceAmount = '';
