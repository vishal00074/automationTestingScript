...
class PortalScriptCDP
{
    ...
    private function processInvoices($paging_count = 1)
    {
        ...
        $rows = $this->exts->querySelectorAll('div#content table.table tbody tr');
        foreach ($rows as $row) {
            $tags = $this->exts->querySelectorAll('td', $row);
            if (count($tags) >= 5 && $this->exts->querySelector('a[href*="download"]', $tags[4]) != null) {
                $invoiceUrl = $this->exts->querySelector('a[href*="download"]', $tags[4])->getAttribute("href");
        ...
        foreach ($invoices as $invoice) {
            ...
            $invoice['invoiceDate'] = $this->exts->parse_date($invoice['invoiceDate'], 'Y-m', 'Y-m-01');
        ...
        if (
            ...
            $this->exts->querySelector('a[id="table-Rechnungen_next"]:not(.disabled)') != null
        ) {
            $paging_count++;
            $this->exts->click_by_xdotool('a[id="table-Rechnungen_next"]:not(.disabled)');
            sleep(5);
            $this->processInvoices($paging_count);
        } else if ($restrictPages > 0 && $paging_count < $restrictPages && $this->exts->querySelector('a[id="table-Rechnungen_next"]:not(.disabled)') != null) {
            $paging_count++;
            $this->exts->click_by_xdotool('a[id="table-Rechnungen_next"]:not(.disabled)');
            sleep(5);
            $this->processInvoices($paging_count);
