...
class PortalScriptCDP
{
    ...
    public $username_selector = 'div[class*="signup_loomSignupEmailForm"] input#email, div[class*="signup_loomSignupComponent"] input#email, form input[type="email"]';
    public $password_selector = 'input#password';

    public $check_login_failed_selector = 'div[class*="email-signup-form_loomMaskedInputContainer"] + span, div.error-bar.error, form input[type=email] + span';
    public $check_login_success_selector = 'a[href="/settings/account"], button[aria-controls="settings-panel"], [class*="profile-bubble_avatarLink"]';
    ...
    private function initPortal($count)
    {
        ...
        if ($this->isExists('ul[class*="header_actions"] img[src*="avatar"]')) {
            $this->exts->log('have avatar');
            $this->exts->moveToElementAndClick('ul[class*="header_actions"] img[src*="avatar"]');
            sleep(4);
        }
        ...
        if ($this->exts->urlContains('/select-workspace')) {
            $this->exts->moveToElementAndClick('a[class*="workspace-selector"]');
            sleep(16);
        }
        ...
        if ($this->exts->getElement($this->check_login_success_selector) != null) {
            ...
            if (strpos(strtolower($this->exts->extract($this->check_login_failed_selector, null, 'innerText')), 'your login information was incorrect') !== false) {
                $this->exts->loginFailure(1);
            } else if (strpos(strtolower($this->exts->extract($this->check_login_failed_selector, null, 'innerText')), 'unable to log this user in at this time') !== false) {
                $this->exts->loginFailure(1);
            } else if (strpos(strtolower($this->exts->extract('#email-signup-button[type="submit"]', null, 'innerText')), 'create free account') !== false) {
                $this->exts->capture('Account-not-register');
                $this->exts->loginFailure(1);
            } else if (strpos(strtolower($this->exts->extract('.signup_loomSignupComponentWrap_cao', null, 'innerText')), 'sent a verification link to the email you provided') !== false || $this->isExists('button[id="resendVerificationEmailButton"]')) {
                $this->exts->account_not_ready();
            } else if (strpos(strtolower($this->exts->extract('div[class*="select-workspace_container"]', null, 'innerText')), 'this account is deactivated in the following workspaces') !== false) {
                $this->exts->account_not_ready();
            } else if (stripos($this->exts->extract('.select-workspace_container_1Y1  > div > div > span[font-weight="bold"]'), 'Your account was deactivated') !== false) {
                $this->exts->account_not_ready();
            } else if ($this->isExists('div[direction="column"] > span')) {
                $this->exts->log('Your login information was incorrect.');
                $this->exts->loginFailure(1);
            } else {
    ...
    private function checkFillLogin()
    {
        ...
        $this->exts->log('login_with_google_value ' . $this->login_with_google);

        $this->exts->log('login_with_apple_value ' . $this->login_with_apple);

        if ($this->login_with_google == 1) {
            $this->exts->moveToElementAndClick('button#googleOAuthButton, button[data-testid*="googleOAuthButton"]');
            $this->exts->log('Open Google Login Page');
            sleep(3);
            $this->exts->switchToNewestActiveTab();
            $this->loginGoogleIfRequired();
            $this->exts->capture("after-login-google");
            $tab_buttons = $this->exts->getElements('div.sticky button');
        ...
        } else if ($this->login_with_apple == 1) {
            $this->exts->moveToElementAndClick('button#appleOAuthButton, button[data-testid*="appleOAuthButton"]');
            sleep(5);
            $this->exts->switchToNewestActiveTab();
            $this->loginAppleIfRequired();
            $this->exts->capture("after-login-apple");
            $this->exts->switchToInitTab();
            ...
            $isAtLassianLogin =  $this->exts->extract('#email-signup-button[type="submit"]', null, 'innerText');
            $this->exts->log('isAtLassianLogin:: ' . $isAtLassianLogin);

            if (stripos($isAtLassianLogin, 'Atlassian') !== false) {
            ...
            if ($loginTab != null) {
                $this->exts->switchToTab($loginTab);
                sleep(4);
            }

            if ($this->isExists('button.amzn-captcha-verify-button')) {
                $this->exts->moveToElementAndClick('button.amzn-captcha-verify-button');
                sleep(10);
            }

            if ($this->isExists('button[id="login-submit"]')) {
            ...
            if ($this->isExists($this->password_selector)) {
                ...
                $this->exts->loginFailure(1);
            } else if (stripos($this->exts->extract('#email-signup-button[type="submit"]', null, 'innerText'), 'Apple') !== false) {
                $this->exts->moveToElementAndClick('#email-signup-button[type="submit"]');
                sleep(5);
                $this->exts->switchToNewestActiveTab();
                $this->loginAppleIfRequired();
    ...
    private function loginAppleIfRequired()
    {
        ...
        if ($this->exts->urlContains('apple.com/auth/authorize')) {
            ...
            if ($this->isExists('iframe[name="aid-auth-widget"]')) {
                $this->exts->switchToFrame('iframe[name="aid-auth-widget"]');
            }
            if ($this->isExists('.signin-error #errMsg + a')) {
                $this->exts->loginFailure(1);
            } else if ($this->isExists('iframe[src*="/account/repair"], repair-missing-items, button[id*="unlock-account-"]')) {
                $this->exts->account_not_ready();
            }
            ...
            $this->exts->switchToDefault();
            if ($this->isExists('iframe[src*="/account/repair"]')) {
            ...
            if ($this->isExists('iframe#aid-auth-widget-iFrame')) {
            ...
            if ($this->isExists('.privacy-consent.fade-in button.nav-action')) {
    ...
    private function checkFillAppleLogin()
    {
        ...
        if ($this->exts->getElement($this->apple_password_selector) != null) {
            ...
            sleep(1);
            if ($this->isExists('#remember-me:not(:checked)')) {
            ...
            $this->exts->switchToFrame('iframe[name="aid-auth-widget"]');
            sleep(1);
            if ($this->exts->getElement($this->apple_password_selector) != null && !$this->isExists('.signin-error #errMsg + a')) {
    ...
    private function checkFillAppleTwoFactor()
    {
        ...
        if ($this->isExists('.devices [role="list"] [role="button"][device-id]')) {
            $this->exts->moveToElementAndClick('.devices [role="list"] [role="button"][device-id]');
            sleep(5);
        }
        if ($this->isExists('div#stepEl div.phones div[class*="si-phone-name"]')) {
        ...
        if ($this->exts->getElement('input[id^="char"]') != null) {
            ...
            if (!empty($two_factor_code) && trim($two_factor_code) != '') {
                ...
                if ($this->isExists('.button-bar button:last-child[id*="trust-browser-"]')) {
    ...
    private function processMultiAccount()
    {
        ...
        sleep(5);
        $this->exts->moveToElementAndClick('button#workspaceDropdownHeader');
        sleep(5);
        $menbers = count($this->exts->getElements('div[class*="workspace-selector_workspacesListSection"]'));
        for ($i = 0; $i < $menbers; $i++) {
            $button_menber = $this->exts->getElements('div[class*="workspace-selector_workspacesListSection"]')[$i];
            try {
                $this->exts->log('Click button');
                $button_menber->click();
            } catch (\Exception $exception) {
                $this->exts->log('Click button by javascript');
                $this->exts->execute_javascript("arguments[0].click()", [$button_menber]);
            }
            sleep(15);
            $this->exts->openUrl($this->invoicePageUrl);
            $this->processInvoices();
    ...
    private function processInvoices()
    {
        ...
        $invoices = [];
        if ($this->exts->getElements('//a[contains(text(), "View Details")]/../..', null) != null) {
            $rows = count($this->exts->getElements('//a[contains(text(), "View Details")]/../..', null));
            $this->exts->log('Invoices found: ' . $rows);
            for ($i = 1; $i <= $rows; $i++) {
                $row = $this->exts->getElements('(//a[contains(text(), "View Details")]/../..)[' . $i . ']', null, 'xpath');
                $tags = $this->exts->getElements('(//a[contains(text(), "View Details")]/../..)[' . $i . ']/div', null, 'xpath');
                if (count($tags) >= 2) {
                    $spans = $this->exts->getElements('span', $tags[0]);
                    $invoiceDate = trim($this->getInnerTextByJS($spans[0], $tags[0]));
                    $invoiceDate = trim(preg_replace('/[^\d\\/]/', '', $invoiceDate));
                    $invoiceDate = $this->exts->parse_date($invoiceDate, '', 'Y-m-d');
                    $invoiceAmount = trim(preg_replace('/[^\d\.\,]/', '', $this->getInnerTextByJS($tags[1]))) . ' USD';
                    ...
                    $this->exts->click_element('(//a[contains(text(), "View Details")])[' . $i . ']');
                    sleep(5);
            ...
            $rows = count($this->exts->getElements('//a[contains(text(), "Download")]/../..', null, 'xpath'));
            $this->exts->log('Invoices found: ' . $rows);
            for ($i = 1; $i <= $rows; $i++) {
                $row = $this->exts->getElements('(//a[contains(text(), "Download")]/../..)[' . $i . ']', null, 'xpath');
                $tags = $this->exts->getElements('(//a[contains(text(), "Download")]/../..)[' . $i . ']/div', null, 'xpath');
                if (count($tags) >= 2) {
                    $spans = $this->exts->getElements('span', $tags[0]);
                    $invoiceDate = trim($this->getInnerTextByJS($spans[0], $tags[0]));
                    $invoiceDate = trim(preg_replace('/[^\d\\/]/', '', $invoiceDate));
                    ...
                    $this->exts->click_element('(//a[contains(text(), "Download")])[' . $i . ']');
                    sleep(15);
                    if (!$this->isExists('a[href*=".com/invoice"], button[data-testid="download-invoice-receipt-pdf-button"]')) {
                        sleep(5);
                    }
