<?php // replace exists to isExists function and added condtion to handle empty invoice name
...
class PortalScriptCDP
{
    ...
    // Server-Portal-ID: 36580 - Last modified: 04.08.2025 13:18:07 UTC - User: 1
        ...
        if ($this->exts->getElement($this->check_login_success_selector) != null) {
            ...
            if ($this->download_monthly_payouts == 1) {
                ...
                if ($this->isExists('a#invoices')) {
                    ...
                    $this->exts->waitTillPresent('button[data-selector="CALENDAR_BUTTON"]');
    ...
    {
        $this->exts->waitTillPresent('button[class*="cui-listitem"]', 30);
        $this->exts->capture("4-invoices-page");
        $invoices = [];
        ...
        foreach ($rows as $row) {
            ...
            $this->exts->click_element($row);
            sleep(5);
            $this->exts->waitTillPresent('button[aria-label="More"]');
