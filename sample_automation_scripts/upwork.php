<?php
// Server-Portal-ID: 428 - Last modified: 20.01.2025 11:44:28 UTC - User: 1
...
private function checkFillExtraAuthentication() {
	...
	if($this->exts->exists('[data-ng-if*="isActiveScreen"]:not(.ng-hide) input#login_deviceAuthorization_answer, input[name="login[deviceAuthorization][answer]"], .security-question input#login_answer, #login input#login_answer')){
		...
		$this->exts->moveToElementAndType('reenter-password-form input#sensitiveZone_password, input#reenterPassword_password', $this->exts->password);
...
private function doAfterLogin() {
	...
	if($this->exts->exists($this->check_login_success_selector) || $this->exts->urlContains('upwork.com/freelancers/~')) {
		...
		if($this->exts->urlContains('/create-profile/') || $this->exts->urlContains('/account-recovery/')){
			...
			$this->exts->account_not_ready();
		} else if(strpos(strtolower($this->exts->extract('.up-form-message-error')), 'passw') !== false ||
			strpos(strtolower($this->exts->extract('#username-message')), 'incorrect') !== false) {
			$this->exts->loginFailure(1);
		} else {
...
private function processInvoices() {
	...
	$rows = count($this->exts->getElements('.trx-history table tbody tr, .trx-list table tbody tr'));
	for ($i=0; $i < $rows; $i++) {
		$row = $this->exts->getElements('.trx-history table tbody tr, .trx-list table tbody tr')[$i];
		$tags = $this->exts->getElements('td', $row);
		if(count($tags) >=6 && ($this->exts->getElement('a', end($tags)) != null || $this->exts->getElement('button', end($tags)) != null)) {
			...
			$invoiceName = $ref_id_button->getAttribute('innerText');
			...
			// Only download invoice if it not exisited
			...
			sleep(3);
			if($this->exts->exists('.up-modal-body a[href*=".pdf"], .air3-modal-content a[href*=".pdf"], button[data-qa*="download-invoice-pdf"]')){
				$this->isNoInvoice = false;
				$this->exts->log('--------------------------');
				...
				$this->exts->click_element('.up-modal-body a[href*=".pdf"], .air3-modal-content a[href*=".pdf"], button[data-qa*="download-invoice-pdf"]');
				sleep(5);
