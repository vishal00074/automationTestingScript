...
private function downloadInvoices($count = 0) {
	...
	foreach ($rows as $row) {
		$download_link = $this->exts->getElement('a', $row);
		if($download_link != null) {
			$invoiceUrl = $download_link->getAttribute("href");

            $invoiceName = basename($invoiceUrl) ?? time();
    ...
    foreach ($invoices as $invoice) {
        ...
        $this->exts->waitTillPresent('button[class*="printButton"]');
        
        $downloaded_file = $this->exts->click_and_print('button[class*="printButton"]', $invoiceFileName);
