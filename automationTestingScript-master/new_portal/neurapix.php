...
class PortalScriptCDP
{
    ...
    private function downloadInvoices($count = 1)
    {
        ...
        $this->exts->waitTillPresent('div.accordion div.accordion-item');
        $this->exts->capture("4-invoices-classic");

        $invoices = [];
        $rows = $this->exts->getElements('div.accordion div.accordion-item');
        foreach ($rows as $key => $row) {
            $invoiceLink = $this->exts->getElement('a[href*="invoices"]', $row);
            if ($invoiceLink != null) {
                $invoiceUrl = $invoiceLink->getAttribute("href");
                $invoiceName = $this->exts->extract('p b', $row);
                $invoiceDate = $this->exts->extract('p', $row);
                $invoiceAmount = '';
