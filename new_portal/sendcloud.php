...
class PortalScriptCDP
{
    ...
    public $baseUrl = 'https://app.sendcloud.com/v2/account/financial/invoices/list';
    public $loginUrl = 'https://app.sendcloud.com/v2/account/financial/invoices/list';
    public $invoicePageUrl = 'https://app.sendcloud.com/v2/account/financial/invoices/list';

    public $username_selector = 'input[id="username"]';
    public $password_selector = 'input[id="password"]';
    public $remember_me_selector = '';
    public $submit_login_selector = 'button[name="submit"].account__btn';

    public $check_login_failed_selector = 'div.alert-danger';
    public $check_login_success_selector = 'a[href*="invoices/list"]';
    ...
    private function downloadInvoices($count = 1)
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            if ($invoiceLink != null) {
                ...
                $invoiceName = $this->exts->extract('td:nth-child(1) div[data-test="invoice-ref"]', $row);
        ...
        foreach ($invoices as $invoice) {
            ...
            sleep(2);
            $this->waitFor('a[data-test="invoice-download-pdf"]');

            if ($this->exts->querySelector('a[data-test="invoice-download-pdf"]')) {
                $downloaded_file = $this->exts->click_and_download('a[data-test="invoice-download-pdf"]', 'pdf', $invoiceFileName);
                sleep(5);
        ...
        $pagiantionSelector = 'li button[data-test="pagination-next-page"]:not(:disabled)';
        if ($restrictPages == 0) {
