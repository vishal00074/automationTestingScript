...
class PortalScriptCDP
{
    ...
    public $baseUrl = 'https://fr.fashionnetwork.com/societeCompte/orders/';
    public $loginUrl = 'https://fr.fashionnetwork.com/societeCompte/orders/';
    public $invoicePageUrl = 'https://fr.fashionnetwork.com/societeCompte/orders/';
    ...
    public $check_login_success_selector = 'a[href*="v2-logout/"]';
    ...
    private function downloadInvoices($count = 0)
    {
        ...
        $this->exts->waitTillPresent('div.tw-border-grey-light');
        $this->exts->capture("4-invoices-classic");

        $invoices = [];
        $rows = $this->exts->getElements('div.tw-border-grey-light');
        foreach ($rows as $key => $row) {
            $invoiceLink = $this->exts->getElement('a', $row);
            if ($invoiceLink != null) {
                $invoiceUrl = $invoiceLink->getAttribute("href");
                $invoiceName = $this->exts->extract('div.tw-grid:nth-child(2)', $row);
                $invoiceDate = $this->exts->extract('div:nth-child(1) span:nth-child(2)', $row);
                $invoiceAmount = $this->exts->extract('div.tw-grid:nth-child(4)', $row);
        ...
        $pagiantionSelector = 'ul.page-nav__list li.page-nav__item.page-nav__item--icon a[rel="next"]';
        if ($restrictPages == 0) {
