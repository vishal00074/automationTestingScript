    ...
    private function downloadInvoices($count = 1)
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            if ($invoiceLink != null) {
                ...
                $invoiceAmount = $this->exts->extract('td:nth-child(7)', $row);
        ...
        if ($count < $restrictPages && $this->exts->exists('nav[aria-label="pagination"] a[class*="action  next"]')) {
            $this->exts->moveToElementAndClick('nav[aria-label="pagination"] a[class*="action  next"]');
            sleep(7);
            $count++;
