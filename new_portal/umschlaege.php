...
class PortalScriptCDP
{
    ...
    private function initPortal($count)
    {
        ...
        if ($this->checkLogin()) {
            ...
            if ($this->exts->exists('aside a[href*="xCmd=rechnungen"]')) {
                $this->exts->moveToElementAndClick('aside a[href*="xCmd=rechnungen"]');
                sleep(2);
            }
            ...
            if (
                stripos($error_text, strtolower('Ihre E-Mail-Adresse und das Passwort konnten in der Datenbank nicht gefunden werden.')) !== false ||
    ...
    private function downloadInvoices($count = 1)
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            if ($invoiceLink != null) {
                ...
                $invoiceName = $queryParams['i'] ?? time(); //create custom name in case invoice name not found
